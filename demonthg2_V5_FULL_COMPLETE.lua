...
-- ===== ANTI KILL HYBRID LOGIC - FIXED & COMPLETE =====
local deathKeywords = {"Dead", "Death", "Kill", "Jumpscare", "Down", "Die"}
local LocalPlayer = game:GetService("Players").LocalPlayer

-- Hook Metatable chuẩn vào game để chặn RemoteEvent từ Server
local function HookRemoteCalls()
    ...
    if success and mt then
        ...
        mt.__namecall = newcclosure(function(self, ...)
            ...
            if _G.antiKillActive and (method == "FireServer" or method == "InvokeServer") then
                local remoteName = tostring(self)
                for _, keyword in ipairs(deathKeywords) do
                    if string.find(remoteName, keyword, 1, true) then
                        return nil
                    end
                end
            end
...
-- Vòng lặp bảo vệ Humanoid ở Client tốc độ cao
function ApplyHumanoidProtection(char)
    ...
    if humanoid then
        ...
        task.spawn(function()
            while char and char.Parent and _G.antiKillActive do
...
-- Connect CharacterAdded event để tự động bảo vệ khi hồi sinh
LocalPlayer.CharacterAdded:Connect(function(newChar)
    if _G.antiKillActive then
        task.wait(0.5)
        ApplyHumanoidProtection(newChar)
    end
end)

-- Khởi tạo hook ngay khi script load
HookRemoteCalls()
