...
if (GameState == "MainGame") then
    ...
    local flag_4 = false
    -- Map tên evidence -> tên node trong Journal (EvidenceTypes)
    local evidenceNodeMap = {
        ["EMF Level 5"]          = {"EMFLevel5", "EMF Level 5", "EMF5", "EMFReader"},
        ["Handprints"]           = {"Handprints", "HandPrints", "Fingerprints"},
        ["Spirit Box"]           = {"SpiritBox", "Spirit Box"},
        ["Ghost Orb"]            = {"GhostOrb", "Ghost Orb", "GhostOrbs"},
        ["Freezing Temperatures"]= {"FreezingTemperatures", "Freezing Temperatures", "Freezing"},
        ["Ghost Writing"]        = {"GhostWriting", "Ghost Writing", "GhostBook"},
        ["Laser Projector"]      = {"LaserProjector", "Laser Projector", "DOTSProjector", "DOTS"},
        ["Wither"]               = {"Wither", "WitheredFlowers", "Withered Flowers"},
    }
    local function findEvidenceNode(parent_node, evidenceName)
        -- Thử map trực tiếp
        local candidates = evidenceNodeMap[evidenceName]
        if candidates then
            for _, nodeName in ipairs(candidates) do
                local found = parent_node:FindFirstChild(nodeName)
                if found then return found end
            end
        end
        -- Fallback: tìm theo tên gần giống (case-insensitive)
        local lowerTarget = string.lower(evidenceName:gsub(" ", ""))
        for _, child in ipairs(parent_node:GetChildren()) do
            if string.lower(child.Name:gsub(" ", "")) == lowerTarget then
                return child
            end
        end
        -- Fallback cuối: tìm partial match
        for _, child in ipairs(parent_node:GetChildren()) do
            if string.find(string.lower(child.Name), string.lower(evidenceName:sub(1,5))) then
                return child
            end
        end
        return nil
    ...
    local function innerFn_5(a_11, b_11)
        ...
        local value_24 = nil
        if not b_11 then
            -- Mark evidence trong Journal
            local node = findEvidenceNode(child_22, a_11)
            if node then
                local container = node:FindFirstChild("Container")
                if container then
                    value_24 = container:FindFirstChild("Detection")
                end
                if not value_24 then
                    -- Thử tìm Detection trực tiếp
                    value_24 = node:FindFirstChildOfClass("TextButton") or node:FindFirstChild("Detection")
                end
            end
        else
            ...
            if child_2 then
                local ghostNode = child_2:FindFirstChild(a_11)
                if ghostNode then
                    value_24 = ghostNode:FindFirstChild("Detection")
                end
            end
        end
        if not value_24 then
