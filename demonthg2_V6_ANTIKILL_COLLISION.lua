...
-- ===== ANTI KILL - COLLISION REMOVAL (XÓA VA CHẠM) =====
local Workspace = game:GetService("Workspace")
local LocalPlayer = game:GetService("Players").LocalPlayer

-- Tìm Ghost model trong Workspace
local function FindGhost()
    local ghost = Workspace:FindFirstChild("Ghost")
    return ghost
end

-- Xóa TouchInterest/TouchTransmitter từ một Part
...
-- Set CanTouch = false cho một Part
local function DisableCanTouch(part)
    if not part then return end
    pcall(function()
        if part:IsA("BasePart") then
            part.CanTouch = false
...
-- Xóa toàn bộ va chạm từ Ghost
local function DisableGhostCollision(ghost)
    if not ghost then return end
    
    for _, part in pairs(ghost:FindFirstChildOfClass("Model") and ghost:GetDescendants() or ghost:GetChildren()) do
        if part:IsA("BasePart") then
            RemoveTouchInterests(part)
            DisableCanTouch(part)
        end
    end
end

-- Xóa toàn bộ va chạm từ Player
...
local function StartAntiKillLoop()
    task.spawn(function()
        while _G.antiKillActive do
            pcall(function()
                local ghost = FindGhost()
                if ghost then
                    DisableGhostCollision(ghost)
                end
                
                DisablePlayerCollision()
            end)
            task.wait(0.1)  -- Kiểm tra mỗi 0.1s
        end
    end)
end

-- Connect UI Toggle
local antiKillToggleOriginal
if antiKillToggle then
    -- Wrapper lại toggle để thêm logic Anti Kill mới
    local oldConnect = antiKillToggle['Activated'].Connect
    antiKillToggle['Activated'].Connect = function(self, callback)
