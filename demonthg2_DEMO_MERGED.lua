        ...
        if autoSpiritActive then
            ...
            task.spawn(function()
                while autoSpiritActive do
                    ...
                    -- ✅ Check ghost hunting status (từ Demo.lua)
                    if Chara and autoSpiritActive and ghostModel:GetAttribute("Hunting") ~= true then
                        -- Ghost không hunt → tele gần ghost để hỏi spirit box
                        Chara:PivotTo(ghostModel:GetPivot() * CFrame.new(0, 0, 10))
                    else
                        -- Ghost đang hunt → tele ra ngoài (TpOutside)
                        if Chara and autoSpiritActive then
                            TpOutside()
                        end
                    end
                    
                    -- ✅ Spam spirit box questions (từ Demo.lua)
                    if tick() - DelaySBTick > 0.5 and autoSpiritActive and ghostModel:GetAttribute("Hunting") ~= true then
                        ...
                        -- ✅ Equip + Fire Spirit Box (từ Demo.lua)
                        if spiritBoxEquipped then
                            spiritBoxEquipped:Activate()
                            task.wait(0.35)
                            FireSpiritBox()  -- ✅ Call Demo function
                        end
                    end
