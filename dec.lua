    ...
    local function innerFn_6(a_12, b_12, c_11, d_11, e_9)
        ...
        if not b_12 then
            ...
            if not e_9 then
                ...
                billboard_2['Size'] = UDim2.new(4, 0, 1, 0)
                billboard_2['SizeOffset'] = Vector2.new(0, 2.5)
                ...
                label_8['Size'] = UDim2.new(1, 0, 0.7, 0)
                ...
                task.wait(0.25)
                local tweenInfo_11 = TweenInfo.new(0.25, Enum['EasingStyle'].Sine, Enum['EasingDirection'].InOut, 0, false, 0)
                local tween_2 = TweenService:Create(frame_10, tweenInfo_11, {["Size"]=UDim2.new(1, 0, 2, 0)})
                tween_2:Play()
                tween_2['Completed']:Connect(function()
                    label_9 = Instance.new("TextLabel", billboard_2)
                    label_9['Name'] = "Evidence"
                    label_9['TextColor3'] = Color3.fromRGB(255, 255, 255)
                    label_9['Size'] = UDim2.new(1, 0, 0.7, 0)
                    label_9['BackgroundTransparency'] = 1
                    label_9['MaxVisibleGraphemes'] = 0
                    label_9['Position'] = UDim2.new(0, 0, 0.99, 0)
                    label_9['TextScaled'] = true
                    label_9['Text'] = d_11
                    for n = 1, 30 do
                        label_9['MaxVisibleGraphemes'] = n
                        task.wait(0.025)
                    end
                    SetTransparency(0.25, label_9, 0)
                end)
            end
            if e_9 then
    ...
    instance_19['Activated']:Connect(function()
        ...
        if flag_6 then
            while flag_6 do
                for k_33, v_33 in pairs(child_21:GetChildren()) do
                    if (v_33:IsA("Model") and v_33['PrimaryPart']) then
                        ...
                        if not v_33['PrimaryPart']:FindFirstChild("Esp2") then
                            innerFn_6(v_33.PrimaryPart, false, v_33:GetAttribute("ItemName"), "Item")
                        end
                    end
                end
                task.wait(0.5)
            ...
            for k_34, v_34 in pairs(child_21:GetChildren()) do
                if (v_34:IsA("Model") and v_34:FindFirstChild("Handle")) then
                    if v_34['PrimaryPart']:FindFirstChild("Esp2") then
                        innerFn_6(v_34.PrimaryPart, true, false, false)
                    end
                    for k_35, v_35 in pairs(v_34:GetDescendants()) do
                        if v_35:IsA("Highlight") then
                            v_35['OutlineTransparency'] = 1
                            task.wait(0.1)
                            v_35:Destroy()
                        end
                    end
    ...
    instance_20['Activated']:Connect(function()
        ...
        if flag_7 then
            ...
            for k_36, v_36 in pairs(Workspace:GetChildren()) do
                if (v_36:FindFirstChild("Humanoid") and (v_36['Name'] ~= "Ghost")) then
