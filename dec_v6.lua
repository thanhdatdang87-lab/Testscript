        ...
        if autoSpiritActive then
            task.spawn(function()
                while autoSpiritActive do
                    ...
                    -- Nếu chưa cầm tool, tìm và cầm Spirit Box
                    if not spiritBoxEquipped and backpack then
                        local spiritBoxTool = backpack:FindFirstChild("Spirit Box")
                        if not spiritBoxTool and character then
                            -- Có thể đang cầm trong tay, kiểm tra
                            spiritBoxTool = character:FindFirstChild("Spirit Box")
                            if spiritBoxTool then
                                spiritBoxEquipped = true
                            end
                        end
                        
                        if not spiritBoxTool then
                            -- Tìm Spirit Box item và cầm
                            local spiritBox = nil
                            for _, item in pairs(child_21:GetChildren()) do
                            ...
                            if spiritBox and spiritBox.PrimaryPart then
                                -- Tele tới Spirit Box
                                if HumanoidRootPart then
                                ...
                                -- Cầm Spirit Box (proximity prompt)
                                if spiritBox:FindFirstChild("ProximityPrompt") then
                                    task.wait(0.1)
                                    fireproximityprompt(spiritBox.ProximityPrompt)
                                    task.wait(0.8) -- Đợi cầm xong
                                    spiritBoxEquipped = true
                                end
                            end
                        else
                    ...
                    -- Nếu đã cầm tool, hỏi ma liên tục
                    if spiritBoxEquipped and backpack then
                        local tool = backpack:FindFirstChild("Spirit Box")
                        if not tool and character then
                            tool = character:FindFirstChild("Spirit Box")
                        end
                        
                        if tool then
                            -- Cách 1: tool:Activate() nếu là Tool object
                            if tool:IsA("Tool") then
                                ...
                                -- Cách 2: Tìm và click button trong tool GUI
                                local gui = tool:FindFirstChildOfClass("BillboardGui") or tool:FindFirstChild("GUI")
                                if gui then
                    ...
                    task.wait(0.8) -- Delay giữa các lần hỏi (0.8s)
                end
                spiritBoxEquipped = false
