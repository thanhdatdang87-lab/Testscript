...
if (GameState == "MainGame") then
    ...
    -- ===== AUTO SPIRIT BOX HELPER FUNCTIONS (từ Demo.lua) =====
    local DelaySBTick = tick()
    
    local function TpOutside()
        local success, err = pcall(function()
            local pegboard = workspace:FindFirstChild("Map") and workspace:FindFirstChild("Map"):FindFirstChild("Rooms") and workspace:FindFirstChild("Map"):FindFirstChild("Rooms"):FindFirstChild("Base Camp") and workspace:FindFirstChild("Map"):FindFirstChild("Rooms"):FindFirstChild("Base Camp"):FindFirstChild("Pegboard")
            ...
            if union and char and char:FindFirstChild("HumanoidRootPart") then
                ...
                -- Fallback: tele ra xa xôi
                if char and char:FindFirstChild("HumanoidRootPart") then
                    char.HumanoidRootPart.CFrame = CFrame.new(300, 100, 300)
                end
            end
        end)
        if not success then CreateKeybind("Hunt TP failed", 2) end
    end
    ...
    local function FireSpiritBox()
        ...
        if replicatedStorage and replicatedStorage:FindFirstChild("Events") then
            local events = replicatedStorage:FindFirstChild("Events")
            if events and events:FindFirstChild("AskSpiritBoxFromUI") then
                events:FindFirstChild("AskSpiritBoxFromUI"):FireServer(args[math.random(1, #args)])
            end
        end
    end
    -- =========================================================
        ...
        if autoSpiritActive then
            ...
            task.spawn(function()
                while autoSpiritActive do
                    ...
                    if not Chara or not ghostModel then
                        task.wait(0.5)
                        continue
                    end
                    
                    -- ✅ Tele gần ghost (luôn, không check hunting)
                    Chara:PivotTo(ghostModel:GetPivot() * CFrame.new(0, 0, 10))
                    task.wait(0.2)
                    
                    -- ✅ SPAM SPIRIT BOX (main function)
                    if tick() - DelaySBTick > 0.5 then
                        DelaySBTick = tick()
                        
                        -- Tìm Spirit Box trong inventory
                        local spiritBoxEquipped = Chara:FindFirstChild("Spirit Box")
                        
                        -- Nếu không có, tìm và pick up
                        if not spiritBoxEquipped and workspace:FindFirstChild("Items") then
                            for _, item in ipairs(workspace.Items:GetChildren()) do
                                if item:IsA("Model") and item:GetAttribute("ItemName") == "Spirit Box" then
                                    Chara:PivotTo(item:GetPivot() + Vector3.new(0, 3, 0))
                                    task.wait(0.3)
                                    -- ✅ Pick up (press E)
                                    ...
                                    task.wait(0.5)
                                    spiritBoxEquipped = Chara:FindFirstChild("Spirit Box")
                                    break
                                end
                            end
                        end
                        
                        -- ✅ Equip + Fire Spirit Box
                        if spiritBoxEquipped then
                            spiritBoxEquipped:Activate()
                            task.wait(0.35)
                            -- ✅ CALL FIRE SPIRIT BOX (từ Demo.lua)
                            FireSpiritBox()
                            CreateKeybind("Asked Spirit Box", 1)
                        end
                    end
                    
                    -- ✅ CHECK HUNT - nếu hunt thì tele ra ngoài
                    if ghostModel:GetAttribute("Hunting") == true then
                        TpOutside()
                        CreateKeybind("Ghost hunting! Teleporting outside...", 2)
                    end
