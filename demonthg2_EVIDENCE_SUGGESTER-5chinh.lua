...
function FireSpiritBox()
    local args = {
        "Are you far away?", "Are you near?", "Where are you?",
        "What do you want?", "When did you cross over?", "Are you in the room with me?",
        "Do you want us to leave?", "When did you pass away?", "What is your goal?",
        "Why are you here?", "How long ago did you die?", "Is there a ghost here?"
    }
...
function UseSpiritBox()
    ...
    if tick() - DelaySBTick > 0.5 and autoSpiritActive and ghostModel:GetAttribute("Hunting") ~= true then
        ...
        if not Found then
            local FoundItem, Model = FindItem("Spirit Box")
            if FoundItem and Model then
                PickupItem(Model)
                task.wait(0.35)
                ActiveItem()
                task.wait(0.5)
                local Found2, InvSlotNum2 = CheckInventory("Spirit Box")
                if Found2 then
