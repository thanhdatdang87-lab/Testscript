...
if (GameState == "MainGame") then
    ...
    -- ╔════════════════════════════════════════════════════════╗
    -- ║  TELE ALL ITEMS - SUB-BUTTON + CONTINUOUS FARMING     ║
    -- ╚════════════════════════════════════════════════════════╝
    ...
    local farmingState = {
        ...
        startPos = nil,
        droppedPos = {},
        subButtonCreated = false
    }
    
    -- [RESET CAMERA]
    local function resetFarmCamera()
        Camera.CameraType = Enum.CameraType.Custom
        task.wait(0.03)
        if farmingState.character then
            Camera.CameraSubject = farmingState.character:FindFirstChild("Humanoid")
        end
        task.wait(0.03)
    end
    
    -- [VỨT ĐỒ]
    local function dropFarmItems()
        for i = 1, 3 do
            if not farmingState.isRunning then break end
    ...
    -- [QUÉT ITEMS KHÔNG BỊ VỨT]
    local function getFarmItems()
        local items = {}
        local hrp = farmingState.character and farmingState.character:FindFirstChild("HumanoidRootPart")
        ...
        for _, v in ipairs(workspace:GetDescendants()) do
            if v:IsA("Model") and tonumber(v.Name) and v.Name ~= "Fortune Teller" then
                ...
                if prompt and part and prompt.Enabled then
                    ...
                    for _, dropPos in ipairs(farmingState.droppedPos) do
    ...
    local function continuousFarmLoop()
        task.spawn(function()
            farmingState.isRunning = true
            farmingState.character = LocalPlayer.Character
            farmingState.startPos = farmingState.character:FindFirstChild("HumanoidRootPart").CFrame
            
            local firstTime = true
            
            while farmingState.isRunning do
                -- VỨT items cũ (nếu không phải lần đầu)
                ...
                for i = 1, grabCount do
                    ...
                    local currentItems = getFarmItems()
                    if #currentItems == 0 then break end
                    local target = currentItems[1]
                    
                    -- TELE tới item
                    farmingState.character:PivotTo(CFrame.new(target.Position + Vector3.new(0, 0.5, 0.5)))
                    task.wait(0.1)
                ...
                -- TELE VỀ + VỨT
                farmingState.character:PivotTo(farmingState.startPos)
                task.wait(0.2)
                resetFarmCamera()
                task.wait(0.1)
                table.insert(farmingState.droppedPos, farmingState.startPos.Position)
    ...
    -- [LOADING SCREEN]
    local function showFarmLoading()
        ...
        txt.Text = "đang quét map để đảm bảo không nhặt sót item vui lòng đợi...."
        ...
        local bar = Instance.new("Frame", progressBg)
        bar.BackgroundColor3 = Color3.fromRGB(0, 200, 100)
        
        local percent = Instance.new("TextLabel", progressBg)
    ...
    -- [CREATE SUB BUTTON]
    local function createFarmSubButton()
        if farmingState.subButtonCreated then return end
        farmingState.subButtonCreated = true
        ...
        btn.Position = UDim2.new(0.5, -75, 0.25, 0)
        ...
        closeBtn.Position = UDim2.new(0.5, 80, 0.25, 0)
        closeBtn.Text = "✕"
        ...
        btn.MouseButton1Click:Connect(function()
            continuousFarmLoop()
            gui:Destroy()
            farmingState.subButtonCreated = false
        end)
