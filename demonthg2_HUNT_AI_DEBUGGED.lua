...
if (GameState == "MainGame") then
    ...
    local currentHuntStart = nil
    
    -- ===== HUNT PATTERN AI V4 - REAL VERIFICATION LAYERS (IN OTHER TAB) =====
    local huntHistory = {}
    local HuntDataLabel = CreateLog("Hunt Data: 0 hunts", frame_9, 1, 170, false, nil, false, false)
    local HuntPatternLabel = CreateLog("Pattern: Analyzing", frame_9, 1.3, 170, false, nil, false, false)
    local HuntPredictionLabel = CreateLog("Next Hunt: --:--", frame_9, 1.6, 170, false, nil, false, false)
    local HuntAccuracyLabel = CreateLog("Confidence: 0%", frame_9, 1.9, 170, false, nil, false, false)
    
    -- ===== VERIFY EACH HUNT PREDICTION =====
    local function VerifyHuntPrediction(lastHuntDuration, currentInterval, lastInterval, variance, sanityRate)
        local passedLayers = 0
        local totalLayers = 5
        local details = {}
        
        -- LAYER 1: Time Difference Check
        if currentInterval and lastInterval then
            local timeDiff = math.abs(currentInterval - lastInterval)
            if timeDiff < (currentInterval * 0.15) then  -- Within 15%
                passedLayers = passedLayers + 1
                table.insert(details, "✅ L1: Time Match")
            else
                table.insert(details, "❌ L1: Time Diff")
            end
        end
        
        -- LAYER 2: Variance Check
        if variance then
            local avgInterval = (currentInterval + lastInterval) / 2
            if variance < (avgInterval * 0.12) then  -- Variance < 12% of average
                passedLayers = passedLayers + 1
                table.insert(details, "✅ L2: Low Variance")
            else
                table.insert(details, "❌ L2: High Variance")
            end
        end
        
        -- LAYER 3: Pattern Consistency (3+ same pattern)
        if #huntHistory >= 3 then
            local intervals = {}
            for i = 2, math.min(5, #huntHistory) do
                local interval = huntHistory[i].startTime - huntHistory[i-1].endTime
                if interval > 0 then
            ...
            if #intervals >= 2 then
                ...
                for i = 2, #intervals do
                    table.insert(diffs, math.abs(intervals[i] - intervals[i-1]))
                end
                local avgDiff = 0
                for _, d in ipairs(diffs) do avgDiff = avgDiff + d end
                avgDiff = avgDiff / #diffs
                
                if avgDiff < (currentInterval * 0.1) then  -- Consistent pattern
                    passedLayers = passedLayers + 1
                    table.insert(details, "✅ L3: Consistent Pattern")
                else
                    table.insert(details, "❌ L3: Inconsistent Pattern")
                end
            end
        end
        
        -- LAYER 4: Sanity-Based Verification
        if sanityRate and sanityRate > 0 then
            -- Calculate expected hunt time from sanity threshold (50% for medium)
            local expectedSanityHunt = (50 / math.max(sanityRate, 0.5)) * 60  -- Convert to seconds
            if currentInterval then
                local huntTimeDiff = math.abs(expectedSanityHunt - currentInterval)
                if huntTimeDiff < (currentInterval * 0.2) then  -- Within 20%
                    passedLayers = passedLayers + 1
                    table.insert(details, "✅ L4: Sanity Match")
                else
                    table.insert(details, "❌ L4: Sanity Mismatch")
                end
            end
        end
        
        -- LAYER 5: Duration Check (hunt duration stable)
        if #huntHistory >= 2 then
            local lastHuntDur = huntHistory[#huntHistory].duration or 0
            if lastHuntDuration and lastHuntDur > 0 then
                local durationDiff = math.abs(lastHuntDuration - lastHuntDur)
                if durationDiff < 10 then  -- Duration within 10s
                    passedLayers = passedLayers + 1
                    table.insert(details, "✅ L5: Duration Stable")
                else
                    table.insert(details, "❌ L5: Duration Unstable")
                end
            end
        end
        
        -- Calculate confidence based on REAL verification
        local confidence = (passedLayers / totalLayers) * 100
    ...
    local function PredictNextHuntPerHunt()
        if #huntHistory < 1 then return nil end  -- Change từ 2 → 1
        
        if #huntHistory == 1 then
            -- Chỉ có 1 hunt - không thể predict (need 2+)
            return nil
        end
        
        -- Get LAST hunt duration
        local lastHunt = huntHistory[#huntHistory]
        local lastHuntDuration = lastHunt.duration
        
        -- Get LAST interval (Hunt N-1 → Hunt N)
        local lastInterval = lastHunt.startTime - huntHistory[#huntHistory - 1].endTime
        
        -- If we have 3+ hunts, look at previous intervals
        local secondLastInterval = nil
        if #huntHistory >= 3 then
            secondLastInterval = huntHistory[#huntHistory - 1].startTime - huntHistory[#huntHistory - 2].endTime
        end
        
        -- Calculate current prediction
        -- Use last interval OR average of last 2 intervals
        local predictedInterval = lastInterval
        if secondLastInterval then
            predictedInterval = (lastInterval + secondLastInterval) / 2
        end
        
        -- Calculate variance
        local variance = 0
        if secondLastInterval then
            variance = math.abs(lastInterval - secondLastInterval)
        end
        
        -- Determine pattern type
        local patternType = "SINGLE HUNT"
        if #huntHistory >= 3 then
            if variance < (predictedInterval * 0.1) then
                patternType = "VERY STABLE"
            elseif variance < (predictedInterval * 0.2) then
                patternType = "STABLE"
            else
        ...
        -- Get sanity drop rate
        local sanityRate = 0
        local currentSanity = tonumber(LocalPlayer:GetAttribute("Sanity") or 100)
        if currentSanity < 100 then
            sanityRate = (100 - currentSanity) / (tick() - (huntHistory[1].startTime or tick()))
        end
        
        -- VERIFY prediction
        local verification = VerifyHuntPrediction(lastHuntDuration, lastInterval, secondLastInterval, variance, sanityRate)
        
        return {
            prediction = predictedInterval,
            pattern = patternType,
            verification = verification,
            lastInterval = lastInterval,
            secondLastInterval = secondLastInterval
        }
    end
    
    local lastSanityCheck = tick()
    local lastRecordedSanity = 100
    local sanityDropRate = 0
    
    local function UpdateHuntAI()
        local ghostModel = workspace:FindFirstChild("Ghost")
        if not ghostModel then 
            CreateKeybind("⚠️ Ghost not found in workspace!", 1)
            return 
        end
        
        -- Track hunts
        local isHunting = ghostModel:GetAttribute("Hunting") == true
        if isHunting and not lastHuntState then
            currentHuntStart = tick()
            lastHuntState = true
        elseif not isHunting and lastHuntState then
            if currentHuntStart then
                table.insert(huntHistory, {
                    startTime = currentHuntStart,
                    endTime = tick(),
                    duration = tick() - currentHuntStart
                })
                CreateKeybind("✅ Hunt #" .. #huntHistory .. " recorded!", 1)
                if #huntHistory > 20 then
                    table.remove(huntHistory, 1)
                end
            end
            lastHuntState = false
        end
        ...
        -- Sanity rate
        if tick() - lastSanityCheck > 1 then
            local currentSanity = tonumber(LocalPlayer:GetAttribute("Sanity") or 100)
            sanityDropRate = (lastRecordedSanity - currentSanity) / (tick() - lastSanityCheck)
            lastRecordedSanity = currentSanity
            lastSanityCheck = tick()
        end
        
        -- Update UI
        local huntCount = #huntHistory
        HuntDataLabel['Text'] = "Hunt Data: " .. huntCount .. " hunts"
        if huntCount == 0 then
            HuntDataLabel['TextColor3'] = Color3.fromRGB(255, 100, 100)  -- 🔴 No data
        elseif huntCount == 1 then
            HuntDataLabel['TextColor3'] = Color3.fromRGB(255, 165, 0)  -- 🟠 1 hunt (need 2+)
        elseif huntCount < 5 then
            HuntDataLabel['TextColor3'] = Color3.fromRGB(255, 200, 0)  -- 🟡 Growing
        else
            HuntDataLabel['TextColor3'] = Color3.fromRGB(0, 255, 0)  -- 🟢 Good data
        end
        
        local pred = PredictNextHuntPerHunt()
        if pred then
            HuntPatternLabel['Text'] = "Pattern: " .. pred.pattern
            if pred.verification.passedLayers >= 4 then
                HuntPatternLabel['TextColor3'] = Color3.fromRGB(0, 255, 0)
            elseif pred.verification.passedLayers >= 2 then
                HuntPatternLabel['TextColor3'] = Color3.fromRGB(255, 200, 0)
            else
                HuntPatternLabel['TextColor3'] = Color3.fromRGB(255, 0, 0)
            end
            
            -- Calculate time until next hunt
            local timeLeft = pred.prediction - (tick() - huntHistory[#huntHistory].endTime)
            timeLeft = math.max(timeLeft, 5)
            local mins = math.floor(timeLeft / 60)
            local secs = math.floor(timeLeft % 60)
            
            HuntPredictionLabel['Text'] = string.format("Next Hunt: %d:%02d", mins, secs)
            if timeLeft < 60 then
                HuntPredictionLabel['TextColor3'] = Color3.fromRGB(255, 0, 0)
            elseif timeLeft < 180 then
                HuntPredictionLabel['TextColor3'] = Color3.fromRGB(255, 165, 0)
            else
                HuntPredictionLabel['TextColor3'] = Color3.fromRGB(0, 255, 0)
            end
            
            -- Show REAL confidence (not fake %)
            HuntAccuracyLabel['Text'] = string.format("Confidence: %d%% (%d/%d Layers)", 
                math.floor(pred.verification.confidence), 
                pred.verification.passedLayers,
                pred.verification.totalLayers)
                
            -- Color by real verification
            if pred.verification.passedLayers == 5 then
                HuntAccuracyLabel['TextColor3'] = Color3.fromRGB(0, 255, 0)  -- 🟢 All pass
            elseif pred.verification.passedLayers >= 3 then
                HuntAccuracyLabel['TextColor3'] = Color3.fromRGB(255, 200, 0)  -- 🟡 Most pass
            else
                HuntAccuracyLabel['TextColor3'] = Color3.fromRGB(255, 0, 0)  -- 🔴 Few pass
            end
        else
            HuntPatternLabel['Text'] = "Pattern: Need 2+ hunts"
            HuntPatternLabel['TextColor3'] = Color3.fromRGB(255, 0, 0)
            HuntPredictionLabel['Text'] = "Next Hunt: --:--"
            HuntAccuracyLabel['Text'] = "Confidence: 0% (0/5 Layers)"
        end
    end
