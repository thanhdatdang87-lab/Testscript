...
if (GameState == "MainGame") then
    ...
    local flag_4 = false
    -- Debug: in tên thật của tất cả node trong EvidenceTypes khi load
    task.spawn(function()
        task.wait(3) -- đợi Journal load xong
        if child_22 then
            local names = {}
            for _, c in ipairs(child_22:GetChildren()) do
                table.insert(names, c.Name)
            end
            warn("[ESP Debug] EvidenceTypes children:", table.concat(names, ", "))
        end
        local ghostTypesNode = PlayerGui:FindFirstChild("Journal") and
            PlayerGui.Journal:FindFirstChild("Holder") and
            PlayerGui.Journal.Holder:FindFirstChild("Pages") and
            PlayerGui.Journal.Holder.Pages:FindFirstChild("Page4") and
            PlayerGui.Journal.Holder.Pages.Page4:FindFirstChild("Right") and
            PlayerGui.Journal.Holder.Pages.Page4.Right:FindFirstChild("Page") and
            PlayerGui.Journal.Holder.Pages.Page4.Right.Page:FindFirstChild("GhostTypes")
        if ghostTypesNode then
            local gnames = {}
            for _, c in ipairs(ghostTypesNode:GetChildren()) do
                table.insert(gnames, c.Name)
            end
            warn("[ESP Debug] GhostTypes children:", table.concat(gnames, ", "))
        end
    end)
    ...
    local evidenceNodeMap = {
        ["EMF Level 5"]           = {"EMFLevel5", "EMF Level 5", "EMF5", "EMF"},
        ["Handprints"]            = {"Handprints", "HandPrints", "Fingerprints", "UVFingerprints"},
        ["Spirit Box"]            = {"SpiritBox", "Spirit Box", "SpiritualBox"},
        ["Ghost Orb"]             = {"GhostOrb", "Ghost Orb", "GhostOrbs", "Orb"},
        ["Freezing Temperatures"] = {"FreezingTemperatures", "Freezing Temperatures", "Freezing", "FreezeTemp"},
        ["Ghost Writing"]         = {"GhostWriting", "Ghost Writing", "GhostBook", "Writing"},
        ["Laser Projector"]       = {"LaserProjector", "Laser Projector", "DOTSProjector", "DOTS", "Laser"},
        ["Wither"]                = {"Wither", "WitheredFlowers", "Withered Flowers", "WitheredFlower"},
    }
