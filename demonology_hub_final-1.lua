...
if (GameState == "MainGame") then
    ...
    -- ===== ANTI KILL V12 - SMART LOCK HEIGHT WHEN HUNTING IN RANGE =====
    _G.antiKillActive = false
    _G.antiKillRange = 50
    
    local antiKillToggle = CreateToggle("Anti Kill", UI["PlayerSettings"], 10, 185, 85)
    local antiKillRangeSlider = CreateSlider("Anti Kill Range", UI["PlayerSettings"], 11, 185, false, 10, 100, "AntiKillRange")
    
    -- Constants
    local LOCK_HEIGHT = 6
    
    -- Tìm Ghost trong Workspace
    local function FindNearbyGhost(detectionRange)
        ...
        for _, obj in pairs(Workspace:GetChildren()) do
            if obj:IsA("Model") then
                ...
                if humanoid and not isPlayer then
                    ...
                    if ghostHRP then
                        local distance = (playerPos - ghostHRP.Position).Magnitude
    ...
    local function CleanupPlatform()
        ...
        if antiKillPlatform then
            pcall(function() antiKillPlatform:Destroy() end)
            antiKillPlatform = nil
        end
        lockTargetY = nil
    end
    
    local function StartLockHeight()
        CleanupPlatform()
        ...
        -- Teleport lên 6 studs ngay lập tức
        lockTargetY = rootPart.Position.Y + LOCK_HEIGHT
        rootPart.CFrame = CFrame.new(rootPart.Position.X, lockTargetY, rootPart.Position.Z)
        
        -- Tạo sàn tàng hình đỡ chân
        antiKillPlatform = Instance.new("Part")
        antiKillPlatform.Size = Vector3.new(12, 1, 12)
        antiKillPlatform.Anchored = true
        antiKillPlatform.Transparency = 1
        antiKillPlatform.CanCollide = true
        antiKillPlatform.Name = "AntiKillPlatform"
        antiKillPlatform.Parent = Workspace
        antiKillPlatform.Position = Vector3.new(rootPart.Position.X, lockTargetY - 3, rootPart.Position.Z)
        
        -- Vòng lặp giữ sàn đi theo X, Z nhưng khóa Y ở mức +6 studs
        antiKillConnection = game:GetService("RunService").Heartbeat:Connect(function()
            ...
            local char = LocalPlayer.Character
            if not char or not char:FindFirstChild("HumanoidRootPart") or not antiKillPlatform then
                CleanupPlatform()
                return
            end
            
            local rootPart = char.HumanoidRootPart
            local detectionRange = _G.antiKillRange or 50
            
            -- Tìm Ghost + check 2 điều kiện
            local ghost, distance = FindNearbyGhost(detectionRange)
            
            local shouldActivate = false
            if ghost and distance < detectionRange then
                -- Check điều kiện 1: Ghost đang hunt
                pcall(function()
            ...
            if shouldActivate then
                -- Điều kiện thoả: HUNT + IN RANGE → Giữ lock height
                antiKillPlatform.Position = Vector3.new(rootPart.Position.X, lockTargetY - 3, rootPart.Position.Z)
                antiKillPlatform.CanCollide = true
            else
                -- Không thoả: Thả platform cho player rơi xuống
                if antiKillPlatform then
                    antiKillPlatform.CanCollide = false
                end
            end
        end)
    end
    ...
    antiKillRangeSlider.Changed:Connect(function(value)
        _G.antiKillRange = tonumber(value) or 50
    end)
