...
if (GameState == "MainGame") then
    ...
    debug_list_items['Activated']:Connect(function()
        print("\n" .. string.rep("=", 50))
        print("📍 LISTING ALL ITEMS IN WORKSPACE")
        print(string.rep("=", 50))
        
        local allItems = {}
        
        -- Search trong Workspace.Items
        if child_21 then
            print("\n🔍 In Workspace.Items:")
            for _, item in pairs(child_21:GetChildren()) do
                ...
                print(string.format("  - %s [%s] (parts: %d)", item.Name, itemType, partCount))
                table.insert(allItems, item)
            end
        end
        
        -- Search toàn bộ workspace cho items (tránh bỏ sót)
        print("\n🔍 Tìm tất cả BasePart/Model trong Workspace:")
        for _, obj in pairs(Workspace:GetDescendants()) do
            if (obj:IsA("BasePart") or obj:IsA("Model")) and 
               obj.Name ~= "Baseplate" and 
               obj.Name ~= "Terrain" and
               not obj:IsDescendantOf(LocalPlayer.Character or {}) then
                
                local parent = obj.Parent
                if parent and parent.Name ~= "Workspace" and parent.Name ~= "Items" then
                    print(string.format("  - %s [%s] (Parent: %s)", obj.Name, obj.ClassName, parent.Name))
                end
            end
        end
        
        print(string.format("\n✅ Total: %d items in Workspace.Items", #allItems))
        print(string.rep("=", 50) .. "\n")
    end)
    instance_tele_items['Activated']:Connect(function()
        ...
        -- 🔥 STEP 1: Lọc items chưa có ai cầm + get PrimaryPart
        for _, item in pairs(child_21:GetChildren()) do
            ...
            -- Check xem item có người cầm không
            if primaryPart then
                local isHeld = false
                for _, plr in pairs(Players:GetPlayers()) do
                ...
                if not isHeld then
                    table.insert(itemsToTele, {item = item, part = primaryPart})
                end
            end
        end
        
        -- 🔥 STEP 2: Teleport từng item với smart logic
        for idx, data in ipairs(itemsToTele) do
            local item = data.item
            local primaryPart = data.part
            
            -- Offset ngẫu nhiên để không chồng
            ...
            for heightTry = 1, 3 do
                ...
                -- Raycast check để tránh terrain
                ...
                if rayResult then
                    targetPos = rayResult.Position + Vector3.new(offsetX, 0.5, offsetZ)
                end
                ...
                local checkRay = workspace:Raycast(
                    primaryPart.Position + Vector3.new(0, 0.3, 0),
                    Vector3.new(0, -0.5, 0)
                )
            ...
            -- Delay bé tí để tránh lag
            task.wait(0.08)
        end
        
        CreateKeybind("✅ Teleported " .. count .. "/" .. #itemsToTele .. " items!", 3)
    end)
