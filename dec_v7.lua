        ...
        if AG then
            while AG do
                for ghostName, evidenceList in pairs(value_23) do
                    -- Kiểm tra xem ghost này còn possible không:
                    -- Tất cả evidence đã tìm thấy phải nằm trong evidenceList của ghost đó
                    ...
                    -- Chỉ mark ghost còn possible
                    if isPossible and next(tbl) ~= nil then
        ...
        if autoSpiritActive then
            task.spawn(function()
                while autoSpiritActive do
                    ...
                    -- Kiểm tra đã cầm tool chưa
                    if backpack then
                        local toolInBackpack = backpack:FindFirstChild("Spirit Box")
                        local toolInHand = character and character:FindFirstChild("Spirit Box")
                        if toolInBackpack or toolInHand then
                            spiritBoxEquipped = true
                        end
                    end
                    
                    -- Nếu chưa cầm, tìm item và cầm
                    if not spiritBoxEquipped then
                        local spiritBox = nil
                        for _, item in pairs(child_21:GetChildren()) do
                            if item:IsA("Model") and item:GetAttribute("ItemName") == "Spirit Box" then
                                spiritBox = item
                                break
                            end
                        end
                        
                        if spiritBox and spiritBox.PrimaryPart and HumanoidRootPart then
                            local dist = (spiritBox.PrimaryPart.Position - HumanoidRootPart.Position).Magnitude
                            
                            -- Nếu còn xa, tele gần
                            if dist > 10 then
                                HumanoidRootPart.CFrame = spiritBox.PrimaryPart.CFrame + Vector3.new(0, 3, 0)
                                task.wait(0.3)
                            else
                                -- Đã gần, tìm và click ProximityPrompt
                                local prompt = spiritBox:FindFirstChild("ProximityPrompt")
                                if prompt then
                                    -- Cách 1: Fire prompt directly
                                    fireproximityprompt(prompt, 1)
                                    task.wait(0.2)
                                    
                                    -- Cách 2: Tìm button prompt và click
                                    local promptUI = prompt:FindFirstChild("PromptUI")
                                    if promptUI then
                                        local button = promptUI:FindFirstChildOfClass("TextButton")
                                        if button then
                                    ...
                                    -- Nếu thử quá 5 lần mà vẫn không nhặt được, skip
                                    if pickupAttempts > 5 then
                                        CreateKeybind("Failed to pick up Spirit Box", 2)
                                        autoSpiritActive = false
                                        break
                                    end
                                else
                    ...
                    -- Nếu đã cầm tool, activate để hỏi ma
                    if spiritBoxEquipped then
                        local character = LocalPlayer.Character
                        if character then
                            local tool = character:FindFirstChild("Spirit Box")
                            if not tool then
                                local backpack = LocalPlayer:FindFirstChild("Backpack")
                                if backpack then
                                    tool = backpack:FindFirstChild("Spirit Box")
                                    if tool then
                                        -- Equip tool
                            ...
                            if tool then
                                -- Activate tool để hỏi ma
                                if tool:IsA("Tool") then
                                    tool:Activate()
                                    task.wait(0.5) -- Đợi dialog hiện ra
                                elseif tool:IsA("Model") then
                                    -- Thử tìm và click button bên trong tool
                                    local button = tool:FindFirstChildOfClass("TextButton")
                                    if button then
                                        for _, conn in pairs(getconnections(button.MouseButton1Click)) do
                                            conn:Fire()
                                        end
                    ...
                    task.wait(0.6) -- Delay giữa các lần hỏi
                end
                spiritBoxEquipped = false
