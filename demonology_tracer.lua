...
if (GameState == "MainGame") then
    ...
    -- ===== BOTTOM TRACER WITH COLOR CUSTOMIZATION =====
    _G.bottomTracerActive = false
    _G.rainbowTracerMode = false
    _G.tracerColors = {
        Item = Color3.fromRGB(255, 215, 0),      -- Gold
        Player = Color3.fromRGB(0, 255, 0),     -- Green
        Ghost = Color3.fromRGB(255, 0, 0),      -- Red
        Evidence = Color3.fromRGB(0, 255, 255) -- Cyan
    }
    ...
    local itemTracerSlider = CreateSlider("Item Tracer Color", UI["PlayerSettings"], 13, 185, false, 0, 10, "ItemTracerColor")
    local playerTracerSlider = CreateSlider("Player Tracer Color", UI["PlayerSettings"], 14, 185, false, 0, 10, "PlayerTracerColor")
    local ghostTracerSlider = CreateSlider("Ghost Tracer Color", UI["PlayerSettings"], 15, 185, false, 0, 10, "GhostTracerColor")
    local evidenceTracerSlider = CreateSlider("Evidence Tracer Color", UI["PlayerSettings"], 16, 185, false, 0, 10, "EvidenceTracerColor")
    ...
    local function DrawTracer(from, to, color)
        ...
        part.CanCollide = false
        part.CFrame = CFrame.new((from + to) / 2, to) * CFrame.Angles(0, 0, math.pi / 2)
        part.Size = Vector3.new(0.3, distance, 0.3)
        part.Color = color
        part.TopSurface = Enum.SurfaceType.Smooth
        part.BottomSurface = Enum.SurfaceType.Smooth
        part.Parent = Workspace
        
        game:GetService("Debris"):AddItem(part, 0.016) -- Remove after frame
    ...
    local function StartBottomTracer()
        ...
        tracerConnection = game:GetService("RunService").RenderStepped:Connect(function()
            ...
            local bottomPos = playerPos - Vector3.new(0, 3, 0) -- Draw from bottom (3 studs below)
            
            -- Rainbow mode
            if _G.rainbowTracerMode then
                local hue = (tick() * 0.5) % 1
                local rainbowColor = GetRainbowColor(hue)
                DrawTracer(playerPos, bottomPos, rainbowColor)
            else
                -- Draw tracers for each ESP type
                local tracerColor = _G.tracerColors.Item or colorPresets[1][1]
                DrawTracer(playerPos, bottomPos, tracerColor)
            end
        end)
    end
    
    local function StopBottomTracer()
        if tracerConnection then
            tracerConnection:Disconnect()
            tracerConnection = nil
        end
    ...
    tracerToggle['Activated']:Connect(function()
        _G.bottomTracerActive = not _G.bottomTracerActive
        if _G.bottomTracerActive then
            StartBottomTracer()
        else
            StopBottomTracer()
        end
    ...
    rainbowToggle['Activated']:Connect(function()
        _G.rainbowTracerMode = not _G.rainbowTracerMode
    end)
    
    -- Color slider connections
    itemTracerSlider.Changed:Connect(function(value)
        local colorIndex = tonumber(value) or 1
        _G.tracerColors.Item = colorPresets[math.max(1, math.min(10, colorIndex + 1))][1]
    end)
    
    playerTracerSlider.Changed:Connect(function(value)
        local colorIndex = tonumber(value) or 1
        _G.tracerColors.Player = colorPresets[math.max(1, math.min(10, colorIndex + 1))][1]
    end)
    
    ghostTracerSlider.Changed:Connect(function(value)
        local colorIndex = tonumber(value) or 1
        _G.tracerColors.Ghost = colorPresets[math.max(1, math.min(10, colorIndex + 1))][1]
    end)
    
    evidenceTracerSlider.Changed:Connect(function(value)
        local colorIndex = tonumber(value) or 1
        _G.tracerColors.Evidence = colorPresets[math.max(1, math.min(10, colorIndex + 1))][1]
    end)
