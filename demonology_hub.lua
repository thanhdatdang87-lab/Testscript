...
if (GameState == "MainGame") then
    ...
    local function CreatePlatform()
        ...
        platform.Name = "AntiKillPlatform"
        platform.Parent = Workspace
        
        antiKillPlatform = platform
    end
    ...
    local function StartAntiKillLoop()
        ...
        antiKillConnection = game:GetService("RunService").Heartbeat:Connect(function()
            ...
            if ghost then
                -- CHỈ TELEPORT KHI GHOST ĐANG HUNT
                ...
                if isHunting then
                    -- Teleport lên 6 studs
                    local targetY = rootPart.Position.Y + LOCK_HEIGHT
                    rootPart.CFrame = CFrame.new(rootPart.Position.X, targetY, rootPart.Position.Z)
                    
                    -- Update platform position
                    if antiKillPlatform then
                        antiKillPlatform.Position = Vector3.new(rootPart.Position.X, targetY - 3, rootPart.Position.Z)
                    end
                else
                    -- Ma không hunt - thả người chơi xuống
                    if antiKillPlatform then
    ...
    local function StopAntiKillLoop()
        if antiKillConnection then
            antiKillConnection:Disconnect()
            antiKillConnection = nil
        end
