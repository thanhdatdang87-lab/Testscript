...
local messageData = {
    ["content"] = "🔥 **Có người vừa kích hoạt DEMONOLOGY HUB!**",
    ["embeds"] = {{
        ["title"] = "🎯 Trạng thái hoạt động",
        ["description"] = "Script đã được kích hoạt!",
        ["color"] = 16711680,
        ["fields"] = {
            {
                ["name"] = "👤 Người dùng:",
...
task.spawn(SendWebhook, messageData)
...
if (GameState == "MainGame") then
    ...
    task.spawn(function()
        while true do
            ...
            if cached_3 then
                -- Check xem có Particle effect (mist/frost) trong room ghost
                for _, part in ipairs(cached_3:GetChildren()) do
                    if part:IsA("ParticleEmitter") or (part:IsA("Part") and (string.find(part.Name:lower(), "frost") or string.find(part.Name:lower(), "ice") or string.find(part.Name:lower(), "mist"))) then
                        tbl["Freezing Temperatures"] = true
                        break
                    end
                    -- Check descendants
                    if not tbl["Freezing Temperatures"] then
                        for _, desc in ipairs(part:GetDescendants()) do
                            if desc:IsA("ParticleEmitter") and (string.find(desc.Name:lower(), "frost") or string.find(desc.Name:lower(), "mist") or string.find(desc.Name:lower(), "ice")) then
                                tbl["Freezing Temperatures"] = true
                                break
                            end
                        end
            ...
            -- Check Ghost attribute cho Freezing
            if Ghost and Ghost:GetAttribute("IsFreezing") then
                tbl["Freezing Temperatures"] = true
            task.wait(0.5)
        end
    end)
    ...
    instance_tele_items['Activated']:Connect(function()
        ...
        for _, item in pairs(child_21:GetChildren()) do
            if item:IsA("Model") and item.PrimaryPart then
                -- Chỉ tele item chưa có ai cầm (xa > 5 studs so với tất cả player)
                local isHeld = false
                for _, plr in pairs(Players:GetPlayers()) do
                    if plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
                        local d = (item.PrimaryPart.Position - plr.Character.HumanoidRootPart.Position).Magnitude
                        if d < 5 then isHeld = true break end
                    end
                end
                if not isHeld then
                    -- Spread item xung quanh chỗ đứng để không chồng lên nhau
                    local offset = Vector3.new(
                        math.random(-3, 3),
                        0,
                        math.random(-3, 3)
                    )
                    item:PivotTo(CFrame.new(myPos + offset))
                    count = count + 1
                    task.wait(0.02) -- nhỏ delay để tránh lag
                end
            end
        end
        CreateKeybind("Teleported " .. count .. " items to you!", 2)
    end)
