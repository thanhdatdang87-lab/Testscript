...
if (GameState == "MainGame") then
    ...
    local function innerFn_5(a_11, b_11)
        ...
        if not b_11 then
            ...
            -- Mark ghost type - tìm GhostTypes node
            local ghostTypesNode = nil
            ...
            if journal then
                ...
                if holder then
                    ...
                    if pages then
                        local page4 = pages:FindFirstChild("Page4")
                        if page4 then
                            local right = page4:FindFirstChild("Right")
                            if right then
                                local page = right:FindFirstChild("Page")
                                if page then
                                    ghostTypesNode = page:FindFirstChild("GhostTypes")
                                end
                            end
                        end
                    end
            ...
            -- Cách 2: Fallback - tìm qua GetDescendants
            if not ghostTypesNode then
                if journal then
                    for _, desc in ipairs(journal:GetDescendants()) do
                        if desc.Name == "GhostTypes" and desc:IsA("Folder") then
                            ghostTypesNode = desc
                            break
                        end
                    end
            ...
            if ghostTypesNode then
                findAndFire(ghostTypesNode, a_11, true)
            else
                warn("[Ghost Mark] GhostTypes node not found for:", a_11)
            end
        end
    end
        ...
        if AG then
            while AG do
                ...
                elseif #possibleGhosts > 1 then
                    ...
                    -- Debug: không có ghost nào possible (chỉ hiện nếu có evidence)
                    if evidenceCount > 0 then
                        warn("[Ghost Auto Mark] No possible ghosts found for evidences:", tbl)
                    end
                end
