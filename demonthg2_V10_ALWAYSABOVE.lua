...
if (GameState == "MainGame") then
    ...
    -- ===== ANTI KILL SLIDERS =====
    CreateSlider("Anti Kill Range", UI["PlayerSettings"], 11, 185, false, 10, 100, "AntiKillRange")
    CreateSlider("Anti Kill Height", UI["PlayerSettings"], 12, 185, false, 5, 30, "AntiKillHeight")
    -- =============================
...
local function FindNearbyGhost(detectionRange)
    ...
    -- Duyệt workspace tìm Ghost
    for _, obj in pairs(Workspace:GetChildren()) do
        if obj:IsA("Model") then
            local humanoid = obj:FindFirstChild("Humanoid")
            local isPlayer = false
            
            -- Không phải là Player character
            ...
            -- Nếu là Ghost + có HumanoidRootPart
...
local function TeleportAboveGhost(ghost, heightOffset)
    ...
    pcall(function()
        -- Teleport lên trên đầu Ghost (Y + heightOffset, keep X, Z)
        -- MỖI FRAME = luôn giữ player ở trên
        local teleportPos = ghostHRP.Position + Vector3.new(0, heightOffset, 0)
        playerHRP.CFrame = CFrame.new(teleportPos)
    end)
end

-- Main Loop - Mỗi frame kiểm tra và teleport
local function AntiKillMainLoopV10()
    ...
    -- Lấy parameters từ sliders
    local detectionRange = LocalPlayer:GetAttribute("AntiKillRange") or 50
    local heightOffset = LocalPlayer:GetAttribute("AntiKillHeight") or 15
    
    -- Tìm Ghost gần nhất
    local ghost = FindNearbyGhost(detectionRange)
    
    -- Nếu tìm được Ghost gần → Liên tục teleport lên trên (MỖI FRAME)
    if ghost then
        TeleportAboveGhost(ghost, heightOffset)
    end
end

-- Kết nối vào RunService.Stepped (MỖI KHUNG HÌNH)
local antiKillV10Connection = nil

local function StartAntiKillV10Stepped()
