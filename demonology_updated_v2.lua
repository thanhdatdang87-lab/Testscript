...
if (GameState == "MainGame") then
    ...
    -- ===== BOTTOM TRACER SYSTEM - MULTI-TARGET (UPDATED) =====
    
    -- Config Tracer
    local BottomTracerConfig = {
        Enabled = false,
        RainbowMode = false,
        BottomCenterOrigin = false,  -- Vẽ từ center screen
        ...
        ObjectTypes = {
            ...
            Evidence = {
                ...
                Thickness = 2,
                Transparency = 0.7,
                Patterns = {"Evidence", "EMF", "Thermometer", "Fingerprint"}
            },
            Default = {
                Enabled = true,
    ...
    local function DetectObjectType(object)
        if TracerManager.ObjectCache[object] then
            return TracerManager.ObjectCache[object]
        end
        
        local objectName = object.Name or ""
        local parentName = object.Parent and object.Parent.Name or ""
        
        for objectType, config in pairs(BottomTracerConfig.ObjectTypes) do
            if objectType ~= "Default" then
                for _, pattern in pairs(config.Patterns) do
                    if objectName:match(pattern) or parentName:match(pattern) then
                        TracerManager.ObjectCache[object] = objectType
                        return objectType
                    end
                end
            end
        ...
        TracerManager.ObjectCache[object] = "Default"
        return "Default"
    end
    
    local function GetObjectConfig(objectType)
        return BottomTracerConfig.ObjectTypes[objectType] or BottomTracerConfig.ObjectTypes.Default
    end
    
    local function IsObjectOnScreen(object)
        local humanoidRootPart = object:FindFirstChild("HumanoidRootPart") or object:FindFirstChild("Handle") or object
        if not humanoidRootPart or humanoidRootPart.Parent ~= object then
            humanoidRootPart = object
        end
        
        if not humanoidRootPart.Parent then return false end
        
        local objectPos = humanoidRootPart.Position
        local screenPos, onScreen = Workspace.CurrentCamera:WorldToScreenPoint(objectPos)
        
        if not onScreen then return false end
        
        local screenSize = Workspace.CurrentCamera.ViewportSize
        return screenPos.X > -BottomTracerConfig.ScreenBuffer and 
               screenPos.X < screenSize.X + BottomTracerConfig.ScreenBuffer and
               screenPos.Y > -BottomTracerConfig.ScreenBuffer and 
               screenPos.Y < screenSize.Y + BottomTracerConfig.ScreenBuffer
    end
    ...
    local function GetRainbowColor(hue)
        ...
        local r, g, b = 0, 0, 0
        if hue < 1/6 then r, g, b = c, x, 0
        elseif hue < 2/6 then r, g, b = x, c, 0
        elseif hue < 3/6 then r, g, b = 0, c, x
        elseif hue < 4/6 then r, g, b = 0, x, c
        elseif hue < 5/6 then r, g, b = x, 0, c
        else r, g, b = c, 0, x
        end
    ...
    local function CreateTracerLine(object, objectType)
        local config = GetObjectConfig(objectType)
    ...
    local function UpdateTracerLine(tracerData)
        local object = tracerData.objectInstance
        local line = tracerData.object
        ...
        local objectType = DetectObjectType(object)
        local config = GetObjectConfig(objectType)
        
        if not BottomTracerConfig.ESPEnabled[objectType] then
            line.Visible = false
            return true
        end
        ...
        if not IsObjectOnScreen(object) then
            line.Visible = false
            return true
        end
        ...
        local objectPos = humanoidRootPart.Position
        local screenPos = Workspace.CurrentCamera:WorldToScreenPoint(objectPos)
        
        local screenSize = Workspace.CurrentCamera.ViewportSize
        -- Vẽ từ CENTER SCREEN (giữa màn hình) thay vì đáy
        ...
        -- Rainbow mode
        if BottomTracerConfig.RainbowMode then
            local hue = (tick() * 0.5) % 1
            line.Color = GetRainbowColor(hue)
        else
            line.Color = config.Color
        end
    ...
    local function StartBottomTracerSystem()
        local updateConnection = game:GetService("RunService").RenderStepped:Connect(function()
            if not BottomTracerConfig.Enabled then
                return
            end
            
            local objectsToTracer = {}
            
            if BottomTracerConfig.ESPEnabled.Ghost then
                local ghostModel = Workspace:FindFirstChild("Ghost")
                if ghostModel then
                    table.insert(objectsToTracer, ghostModel)
                end
            end
            
            if BottomTracerConfig.ESPEnabled.Equipment then
                local itemsFolder = Workspace:FindFirstChild("Items")
                if itemsFolder then
                    for _, item in pairs(itemsFolder:GetChildren()) do
                        table.insert(objectsToTracer, item)
                    end
                end
            end
            
            if BottomTracerConfig.ESPEnabled.CursedObject then
                local cursedFolder = Workspace:FindFirstChild("Cursed")
                if cursedFolder then
                    for _, cursed in pairs(cursedFolder:GetChildren()) do
                        table.insert(objectsToTracer, cursed)
                    end
                end
            end
            
            if BottomTracerConfig.ESPEnabled.Evidence then
                local evidenceFolder = Workspace:FindFirstChild("Evidence")
                if evidenceFolder then
                    for _, evidence in pairs(evidenceFolder:GetChildren()) do
                        table.insert(objectsToTracer, evidence)
                    end
                end
            end
            ...
            for i, tracerData in pairs(TracerManager.ActiveTracers) do
                if not UpdateTracerLine(tracerData) then
                    table.insert(removeTracers, i)
                end
            end
            
            for _, index in pairs(removeTracers) do
                table.remove(TracerManager.ActiveTracers, index)
            end
            
            for _, object in pairs(objectsToTracer) do
                local hasTracer = false
                
                for _, tracerData in pairs(TracerManager.ActiveTracers) do
                    if tracerData.objectInstance == object then
                        hasTracer = true
                        break
                    end
                end
                ...
                if not hasTracer then
                    ...
                    if config.Enabled and BottomTracerConfig.ESPEnabled[objectType] then
                        local newTracer = CreateTracerLine(object, objectType)
                        table.insert(TracerManager.ActiveTracers, {
                            object = newTracer,
    ...
    local function StopBottomTracerSystem()
        for _, tracerData in pairs(TracerManager.ActiveTracers) do
            if tracerData.object then
                tracerData.object:Remove()
            end
        end
        TracerManager.ActiveTracers = {}
        TracerManager.ObjectCache = {}
    end
    
    -- UI Color Picker Frames - Thay slider thành ô chọn màu
    local tracerToggle = CreateToggle("Bottom Tracer", UI["PlayerSettings"], 10, 185, 85)
    local rainbowToggle = CreateToggle("Rainbow Tracer", UI["PlayerSettings"], 11, 185, 85)
    
    CreateSectionTitle("Tracer Colors", UI["PlayerSettings"], 12)
    ...
    local function CreateColorPicker(label, parent, position, objectType)
        ...
        for i, colorData in ipairs(colorOptions) do
            ...
            colorBtn.Position = UDim2.new(0, startX + (i - 1) * 16, 0.5, -7)
            ...
            inputConnection = game:GetService("UserInputService").InputBegan:Connect(function(input, gameProcessed)
                ...
                local btnCorner = colorBtn.AbsolutePosition
                local btnEnd = btnCorner + colorBtn.AbsoluteSize
                
                if mousePos.X >= btnCorner.X and mousePos.X <= btnEnd.X and
                   mousePos.Y >= btnCorner.Y and mousePos.Y <= btnEnd.Y and
                   input.UserInputType == Enum.UserInputType.MouseButton1 then
                    
                    BottomTracerConfig.ObjectTypes[objectType].Color = color
                    
                    -- Visual feedback
                    colorBtn.BorderColor3 = Color3.fromRGB(0, 255, 0)
                    task.wait(0.2)
                    colorBtn.BorderColor3 = Color3.fromRGB(255, 255, 255)
                end
            end)
    ...
    CreateColorPicker("Ghost Tracer Color", UI["PlayerSettings"], 13, "Ghost")
    CreateColorPicker("Equipment Tracer Color", UI["PlayerSettings"], 40, "Equipment")
    CreateColorPicker("Cursed Tracer Color", UI["PlayerSettings"], 67, "CursedObject")
    CreateColorPicker("Evidence Tracer Color", UI["PlayerSettings"], 94, "Evidence")
    
    local tracerConnection = nil
    
    tracerToggle['Activated']:Connect(function()
        BottomTracerConfig.Enabled = not BottomTracerConfig.Enabled
        if BottomTracerConfig.Enabled then
            if not tracerConnection then
                tracerConnection = StartBottomTracerSystem()
            end
        else
            StopBottomTracerSystem()
    ...
    rainbowToggle['Activated']:Connect(function()
        BottomTracerConfig.RainbowMode = not BottomTracerConfig.RainbowMode
    end)
    ...
    LocalPlayer.CharacterAdded:Connect(function(newChar)
        if BottomTracerConfig.Enabled then
            task.wait(0.5)
            if not tracerConnection or not tracerConnection.Connected then
                tracerConnection = StartBottomTracerSystem()
            end
        end
    end)
    ...
    local function UpdateEvidenceSuggester()
        ...
        local ghostTypes = value_23 or {
            ...
            ["Doppelganger"] = {"Freezing Temperatures", "Inscription", "Spirit Box"},
            ...
            ["Wretch"] = {"Ghost Orb", "Inscription", "Laser Projector"},
    ...
    instance_12['Activated']:Connect(function()
        ...
        while flag_3 do
            ...
            if (tbl["Inscription"] and not tbl_10["Inscription"]) then
                ...
                task.spawn(CreateKeybind, "New Evidence: Inscription", 2)
            end
