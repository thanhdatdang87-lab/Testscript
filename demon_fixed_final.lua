...
if (GameState == "MainGame") then
    ...
    task.spawn(function()
        while true do
            ...
            -- Cách 2: Detect Spirit Book item (Inscription evidence)
            -- Nếu Spirit Book item tồn tại và có trong room ghost (cached_3), thì ma đã viết
            if cached_3 and child_21 then
                for _, item in pairs(child_21:GetChildren()) do
                    if item:IsA("Model") and item:GetAttribute("ItemName") == "Spirit Book" then
                        -- Check xem Spirit Book có nằm trong favorite room của ma không
