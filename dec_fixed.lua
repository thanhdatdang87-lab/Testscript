            ...
            while flag_6 do
                for k_33, v_33 in pairs(child_21:GetChildren()) do
                    if (v_33:IsA("Model") and v_33['PrimaryPart']) then
                        -- Chỉ ESP item nằm dưới đất, bỏ qua item đang được cầm
                        local primaryPart = v_33['PrimaryPart']
                        local isOnGround = primaryPart and primaryPart.Parent and (primaryPart.Parent == v_33)
                        local isInItemsFolder = v_33.Parent == child_21
                        if isOnGround and isInItemsFolder then
                            ...
                            -- Item đang được cầm: xóa ESP nếu có
