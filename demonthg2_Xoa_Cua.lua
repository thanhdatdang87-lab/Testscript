...
if (GameState == "MainGame") then
    ...
    RemoveDoorButton['Activated']:Connect(function()
        local roomsFolder = workspace:FindFirstChild("Map") and workspace:FindFirstChild("Map"):FindFirstChild("Rooms")
        if not roomsFolder then
            CreateKeybind("Rooms folder not found!", 2)
            return
        end
        ...
        for _, room in ipairs(roomsFolder:GetChildren()) do
            if room:IsA("Folder") or room:IsA("Model") then
                -- Tìm tất cả doors trong room
                for _, obj in ipairs(room:GetDescendants()) do
                    if obj:IsA("Model") and (string.find(obj.Name, "Door") or string.find(obj.Name, "door")) then
                        obj:Destroy()
                        doorCount = doorCount + 1
                    elseif obj:IsA("Part") and (string.find(obj.Name, "Door") or string.find(obj.Name, "door")) then
                        obj:Destroy()
                        doorCount = doorCount + 1
                    end
                end
