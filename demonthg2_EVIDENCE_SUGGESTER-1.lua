...
if (GameState == "MainGame") then
    ...
    local function UpdateEvidenceSuggester()
        local ghostTypes = {
            ...
            ["Oni"] = {"Handprints", "Spirit Box", "EMF Level 5"},
            ["Phantom"] = {"Spirit Box", "Fingerprints", "Freezing Temperatures"},
            ["Poltergeist"] = {"Fingerprints", "Spirit Box", "Freezing Temperatures"},
            ...
            ["Vex"] = {"Spirit Box", "Laser Projector", "Handprints"},
            ["Wendigo"] = {"Ghost Orb", "Inscription", "Laser Projector"},
            ["Wraith"] = {"EMF Level 5", "Fingerprints", "Spirit Box"},
        ...
        if evidenceCount == 0 then
            EvidenceLabel['Text'] = "Need: Chưa có evidence nào"
            EvidenceLabel['TextColor3'] = Color3.fromRGB(255, 0, 0)
        elseif evidenceCount < 3 then
            -- Tìm những evidence cần thiết để đủ 3
            local possibleGhosts = {}
            local neededEvidence = {}
            
            for ghostName, ghostEvs in pairs(ghostTypes) do
                local hasAll = true
                for _, ev in ipairs(ghostEvs) do
                    local found = false
                    for _, currentEv in ipairs(currentEvidence) do
                        if currentEv == ev then
                            found = true
                            break
                        end
                    end
            ...
            -- Tìm những evidence mà nếu thêm sẽ giúp xác định
            local allPossibleEvidence = {}
            for ghostName, ghostEvs in pairs(ghostTypes) do
                local canBe = true
                for _, currentEv in ipairs(currentEvidence) do
                    local found = false
                ...
                if canBe then
                    for _, ghostEv in ipairs(ghostEvs) do
                        ...
                        if not found then
                            ...
                            for _, ev in ipairs(allPossibleEvidence) do
                                if ev == ghostEv then
                                    isInList = true
                                    break
                                end
                            ...
                            if not isInList then
                                table.insert(allPossibleEvidence, ghostEv)
                            end
                        end
                    end
                end
            ...
            local needCount = 3 - evidenceCount
            local evidenceList = table.concat(allPossibleEvidence, " / ")
            
            EvidenceLabel['Text'] = "Need " .. needCount .. " more: " .. evidenceList
            EvidenceLabel['TextColor3'] = Color3.fromRGB(255, 200, 0)  -- 🟡 Yellow
        else
            -- Đủ 3 evidence, hiển thị những evidence đang có
