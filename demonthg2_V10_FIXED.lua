...
if (GameState == "MainGame") then
    ...
    -- ===== ANTI KILL SLIDERS (Position 11-12) =====
    local antiKillRangeSlider = CreateSlider("Anti Kill Range", UI["PlayerSettings"], 11, 185, false, 10, 100, "AntiKillRange")
    local antiKillHeightSlider = CreateSlider("Anti Kill Height", UI["PlayerSettings"], 12, 185, false, 5, 30, "AntiKillHeight")
    -- =============================================
...
-- ===== ANTI KILL V10 - ALWAYS ABOVE GHOST (MỖI FRAME) =====
local RunService = game:GetService("RunService")
local Workspace = game:GetService("Workspace")
local LocalPlayer = game:GetService("Players").LocalPlayer

-- Hàm tìm Ghost gần nhất
local function FindNearbyGhost(detectionRange)

-- ===== ANTI KILL V10 FIXED - ALWAYS ABOVE (MỖI FRAME) =====
local RunService = game:GetService("RunService")
local Workspace = game:GetService("Workspace")
local LocalPlayer = game:GetService("Players").LocalPlayer

-- Global Anti Kill state + settings
_G.antiKillRange = 50
_G.antiKillHeight = 15
...
-- Hàm teleport lên trên đầu Ghost (MỖI FRAME)
local function TeleportAboveGhost(ghost, heightOffset)
    ...
    pcall(function()
        local teleportPos = ghostHRP.Position + Vector3.new(0, heightOffset, 0)
        playerHRP.CFrame = CFrame.new(teleportPos)
    end)
end

-- Main Loop
local function AntiKillMainLoopV10()
    ...
    local heightOffset = _G.antiKillHeight or 15
    
    local ghost = FindNearbyGhost(detectionRange)
    if ghost then
        TeleportAboveGhost(ghost, heightOffset)
    end
end

-- RunService Connection
local antiKillV10Connection = nil

local function StartAntiKillV10()
    if antiKillV10Connection then
        antiKillV10Connection:Disconnect()
    end
    antiKillV10Connection = RunService.Stepped:Connect(function()
        AntiKillMainLoopV10()
    end)
end

local function StopAntiKillV10()
    if antiKillV10Connection then
        antiKillV10Connection:Disconnect()
        antiKillV10Connection = nil
    end
end
...
-- Slider Connections (live update)
if antiKillRangeSlider then
    antiKillRangeSlider.Changed:Connect(function(value)
        _G.antiKillRange = tonumber(value) or 50
    end)
end

if antiKillHeightSlider then
    antiKillHeightSlider.Changed:Connect(function(value)
        _G.antiKillHeight = tonumber(value) or 15
    end)
end
...
LocalPlayer.CharacterAdded:Connect(function(newChar)
    if _G.antiKillActive then
        task.wait(0.5)
        if not antiKillV10Connection or not antiKillV10Connection.Connected then
            StartAntiKillV10()
        end
    end
end)
