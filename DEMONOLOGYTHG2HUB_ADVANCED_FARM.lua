...
if (GameState == "MainGame") then
    ...
    auto_farm_button['Activated']:Connect(function()
        ...
        -- Main farm loop
        for farmLoop = 1, 100 do
            if not child_21 then break end
            
            local itemsPickedThisRound = 0
            
            -- Tele tới TỪNG item một
            for _, item in pairs(child_21:GetChildren()) do
                if itemsPickedThisRound >= 3 then
                    break  -- Đủ 3 items rồi
                end
                ...
                if primaryPart then
                    -- 1️⃣ TELE player tới item
                    HumanoidRootPart.CFrame = primaryPart.CFrame + Vector3.new(0, 2, 0)
                    task.wait(0.3)
                    
                    -- 2️⃣ Try CLICK PICKUP - spam click vào item
                    for clickNum = 1, 5 do
                        -- Simulate click trên item
                    ...
                    elseif Character and Character:FindFirstChild("Backpack") then
                        backpackItems = #Character.Backpack:GetChildren()
                    end
                    
                    if backpackItems > itemsPickedThisRound then
                        itemsPickedThisRound = backpackItems
                        CreateKeybind("📦 Picked up! (" .. itemsPickedThisRound .. "/3)", 1)
                    end
            ...
            -- 4️⃣ Nếu đủ 3 items
            if itemsPickedThisRound >= 3 then
                CreateKeybind("✅ Got 3 items! Returning...", 2)
                task.wait(0.5)
                ...
                -- 5️⃣ DROP items - bấm số (1,2,3 để drop)
                for dropNum = 1, 3 do
                    UserInputService:SendKeyEvent(true, Enum.KeyCode.One, false)
                    task.wait(0.05)
                    UserInputService:SendKeyEvent(false, Enum.KeyCode.One, false)
                    task.wait(0.2)
                end
                
                farmCount = farmCount + 1
                CreateKeybind("🔄 Cycle " .. farmCount .. " - collecting again...", 2)
                task.wait(0.5)
            else
                -- Không đủ items - dừng
                CreateKeybind("❌ Couldn't pick up enough items!", 3)
                break
            end
        end
        ...
        -- Quay về
        HumanoidRootPart.CFrame = CFrame.new(startPos)
        CreateKeybind("🎉 Auto Farm done! " .. farmCount .. " cycles!", 3)
    end)
    debug_list_items['Activated']:Connect(function()
        if not child_21 then return end
        
        print("\n" .. string.rep("=", 60))
        print("📍 ALL ITEMS IN WORKSPACE.ITEMS")
        print(string.rep("=", 60))
        ...
        for _, item in pairs(child_21:GetChildren()) do
            ...
            local hasPrimaryPart = item:IsA("Model") and item.PrimaryPart and "✅" or "❌"
            print(string.format("%d. %s [%s] %s", count, item.Name, itemType, hasPrimaryPart))
        end
        
        print(string.format("\n📊 TOTAL: %d items", count))
        print(string.rep("=", 60) .. "\n")
        
        CreateKeybind("Checked " .. count .. " items - see console!", 2)
    end)
    instance_tele_items['Activated']:Connect(function()
        ...
        -- 🔥 Tele TẤT CẢ items (không filter)
        for _, item in pairs(child_21:GetChildren()) do
            local primaryPart = nil
            
            -- Get PrimaryPart (Model)
            if item:IsA("Model") then
                primaryPart = item.PrimaryPart
                if not primaryPart then
                    for _, desc in pairs(item:GetDescendants()) do
            ...
            if primaryPart then
                ...
                -- Try teleport - 4 lần với height khác nhau
                for tryNum = 1, 4 do
                    local height = 1 + (tryNum - 1) * 1.2
                    local targetPos = myPos + Vector3.new(offsetX, height, offsetZ)
                    ...
                    if rayResult then
                        targetPos = rayResult.Position + Vector3.new(offsetX, 0.8, offsetZ)
                    end
                    ...
                    -- 🔥 STRONG physics reset
                    pcall(function()
                        primaryPart.AssemblyLinearVelocity = Vector3.new(0, 0, 0)
                        primaryPart.AssemblyAngularVelocity = Vector3.new(0, 0, 0)
                        primaryPart.CanCollide = true
                        
                        -- Extra: set CustomPhysicalProperties để tránh collision
                    ...
                    local checkRay = workspace:Raycast(
                        primaryPart.Position + Vector3.new(0, 0.5, 0),
                        Vector3.new(0, -0.8, 0)
                    )
                    
                    if not checkRay or checkRay.Distance > 0.3 then
                        -- Success - item not stuck
                        count = count + 1
                        break
                    end
                    
                    -- Still stuck, try again with higher pos
                end
        ...
        CreateKeybind("✅ Done! Teleported " .. count .. " items!", 3)
    end)
