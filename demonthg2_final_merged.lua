...
if (GameState == "MainGame") then
    ...
    task.spawn(function()
        while true do
            ...
            -- Cách 2: Detect Spirit Book item (Inscription evidence) - SIMPLIFIED
            -- Nếu Spirit Book item tồn tại trong items, tức ma đã/đang viết
            if child_21 then
                for _, item in pairs(child_21:GetChildren()) do
                    if item:IsA("Model") and item:GetAttribute("ItemName") == "Spirit Book" then
                        -- Spirit Book tồn tại = ma đã viết Inscription
                        tbl["Inscription"] = true
                        break
                    end
                end
