...
if (GameState == "MainGame") then
        ...
        while true do
            ...
            if cached_3 then
                for k_28, v_28 in pairs(cached_3:GetDescendants()) do
                    -- Cách 1: Check Decal texture (cũ)
                    if (v_28:IsA("Decal") and (v_28['Texture'] == "rbxassetid://108756644911620")) then
                        tbl["Ghost Writing"] = true
                    end
                end
            end
            -- Cách 2: Detect Ghost Book item (Ghost Writing evidence)
            -- Nếu Ghost Book item tồn tại và có trong room ghost (cached_3), thì ma đã viết
            if cached_3 and child_21 then
                for _, item in pairs(child_21:GetChildren()) do
                    if item:IsA("Model") and item:GetAttribute("ItemName") == "Ghost Book" then
                        -- Check xem Ghost Book có nằm trong favorite room của ma không
                        local ghostFavRoom = Ghost and Ghost:GetAttribute("FavoriteRoom")
                        if ghostFavRoom then
                            local distance = (item.PrimaryPart.Position - cached_3.Position).Magnitude
                            if distance < 50 then -- Nằm trong room ghost
                                -- Nếu book nằm trong room, kiểm tra xem có viết chữ không
                                -- Check pages hoặc textures trong book
                                for _, page in pairs(item:GetDescendants()) do
                                    if page:IsA("Decal") or (page:IsA("SurfaceGui") and page.Name:find("Writing")) then
                                        tbl["Ghost Writing"] = true
                                        break
                                    end
                                end
        ...
        if AG then
            while AG do
                ...
                if #possibleGhosts == 1 then
                    innerFn_5(possibleGhosts[1], true)
    ...
    local instance_auto_spirit = CreateButton("Auto Spirit Box( ĐANG BỊ HƯ )", UI["SCF"], 10.5, 150)
        ...
        if autoSpiritActive then
            task.spawn(function()
                while autoSpiritActive do
                    -- Kiểm tra ghost có đang hunt không
                    if Ghost and Ghost:GetAttribute("Hunting") then
                        CreateKeybind("Ghost is hunting - Auto Spirit stopped", 2)
                        autoSpiritActive = false
                        break
                    end
                    ...
                    -- Kiểm tra xem có cầm Spirit Box không
                    local spiritBoxTool = character:FindFirstChild("Spirit Box")
                    
                    if not spiritBoxTool then
                        -- Kiểm tra lần đầu mới show message
                        if autoSpiritActive then
                            CreateKeybind("Hãy cầm spirit box trên tay và hãy bật nó lên, Hãy đảm bảo rằng bạn đang ở trong phòng con ma và tắt hết đèn trong phòng", 5)
                            autoSpiritActive = false
                        end
                        break
                    end
                    
                    -- Kiểm tra spirit box có bật không (check attribute hoặc enabled state)
                    local isActivated = spiritBoxTool:GetAttribute("Activated") or 
                                       spiritBoxTool:GetAttribute("Enabled") or
                                       spiritBoxTool:GetAttribute("IsActive")
                    
                    -- Nếu là số (1/true) hoặc đơn giản check xem tool có humanoid connection
                    -- Cách khác: check xem có audio/effect khi active
                    local hasActiveGui = spiritBoxTool:FindFirstChildOfClass("BillboardGui") ~= nil
                    
                    if spiritBoxTool and (isActivated or hasActiveGui) then
                        -- Spirit Box đã bật, spam questions
                        -- Tìm button "Ask" hoặc bất kỳ button nào để hỏi ma
                        local buttons = spiritBoxTool:FindFirstChildOfClass("BillboardGui")
                        if buttons then
                            -- Tìm tất cả buttons trong spirit box UI
                            for _, button in ipairs(buttons:GetDescendants()) do
                                if button:IsA("TextButton") then
                                    -- Click tất cả buttons (mỗi button là một câu hỏi)
                            ...
                            -- Fallback: gọi Activate nếu không tìm thấy buttons
                            spiritBoxTool:Activate()
                            task.wait(0.3)
                        end
                    else
                        CreateKeybind("Hãy cầm spirit box trên tay và hãy bật nó lên, Hãy đảm bảo rằng bạn đang ở trong phòng con ma và tắt hết đèn trong phòng", 5)
                    ...
                    task.wait(0.5) -- Delay giữa các lần spam
                end
    ...
    -- [HÀM RESET CAMERA]
    local function fullCameraReset()
        Camera.CameraType = Enum.CameraType.Custom
        task.wait(0.03)
        if autoFarmState.character then
            Camera.CameraSubject = autoFarmState.character:FindFirstChild("Humanoid")
        end
        task.wait(0.03)
    end
    ...
    local function dropAllItems()
        for i = 1, 3 do
            if not autoFarmState.isRunning then break end
    ...
    local function getValidItems()
        local items = {}
        local hrp = autoFarmState.character and autoFarmState.character:FindFirstChild("HumanoidRootPart")
        if not hrp then return items end
        
        for _, v in ipairs(workspace:GetDescendants()) do
            if v:IsA("Model") and tonumber(v.Name) and v.Name ~= "Fortune Teller" then
                local prompt = v:FindFirstChildWhichIsA("ProximityPrompt", true)
                local part = v:FindFirstChildWhichIsA("BasePart")
                
                if prompt and part and prompt.Enabled then
                    local isDropped = false
                    for _, dropPos in ipairs(autoFarmState.droppedPositions) do
                        if (part.Position - dropPos).Magnitude < 6 then
                            isDropped = true
                            break
                        end
                    end
                    if not isDropped then
                        table.insert(items, {Part = part, Dist = (part.Position - hrp.Position).Magnitude})
                    end
                end
            end
        end
        ...
        local final = {}
        for _, item in ipairs(items) do table.insert(final, item.Part) end
        return final
    end
    ...
    local function startAutoFarm()
        task.spawn(function()
            ...
            autoFarmState.character = LocalPlayer.Character
            
            if autoFarmState.character then
                autoFarmState.startPosition = autoFarmState.character:FindFirstChild("HumanoidRootPart").CFrame
            end
            
            if autoFarmState.firstRun then
                dropAllItems()
                task.wait(0.12)
            end
            autoFarmState.firstRun = true
            
            -- Main loop
            local items = getValidItems()
            if #items > 0 then
                local grabCount = math.min(#items, 3)
                for i = 1, grabCount do
                    if not autoFarmState.isRunning then
                        fullCameraReset()
                        break
                    end
                    ...
                    local target = currentItems[1]
                    
                    autoFarmState.character:PivotTo(CFrame.new(target.Position + Vector3.new(0, 0.5, 0.5)))
                    task.wait(0.095)
            ...
            if autoFarmState.startPosition then
                autoFarmState.character:PivotTo(autoFarmState.startPosition)
                task.wait(0.15)
                fullCameraReset()
                task.wait(0.15)
                table.insert(autoFarmState.droppedPositions, autoFarmState.startPosition.Position)
            end
    ...
    local function showLoadingScreen()
        ...
        screenGui.Name = "AutoFarmLoadingGui"
        ...
        bg.BorderColor3 = Color3.fromRGB(100, 100, 100)
        ...
        textLabel.Text = "đang quét map để đảm bảo không nhặt sót item vui lòng đợi...."
        ...
        progressBg.Size = UDim2.new(0.8, 0, 0, 30)
        ...
        progressText.Font = Enum.Font.Arial
        
        -- Animate loading 0% to 100% in 10 seconds
        for i = 0, 100, 2 do
            progressBar.Size = UDim2.new(i / 100, 0, 1, 0)
            progressText.Text = i .. "%"
            task.wait(0.2)  -- 100 steps * 0.2 = 20s, adjust to 10s = 0.1
        end
        ...
        textLabel.Text = "đã quét thành công"
        task.wait(2)
    ...
    -- [BUTTON TELE ALL ITEMS HANDLER]
    instance_tele_items['Activated']:Connect(function()
        if autoFarmState.hasBeenUsed then
            -- Already used once
            CreateKeybind("vui lòng tắt và bật lại ở trong menu để sử dụng 1 lần nữa", 4)
            return
        end
        
        if not autoFarmState.isRunning then
            -- Turn ON - start auto farm
            autoFarmState.hasBeenUsed = true
            startAutoFarm()
        else
            -- Turn OFF - reset
            autoFarmState.isRunning = false
            autoFarmState.hasBeenUsed = false
            autoFarmState.firstRun = false
            autoFarmState.droppedPositions = {}
            fullCameraReset()
            CreateKeybind("đã reset tính năng auto farm", 2)
        end
    end)
