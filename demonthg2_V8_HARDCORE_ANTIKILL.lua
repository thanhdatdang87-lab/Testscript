...
-- ===== ANTI KILL GLOBAL FLAG =====
_G.antiKillActive = false
-- ==================================
...
if (GameState == "MainGame") then
    ...
    -- ===== ANTI KILL TOGGLE (Position 10) =====
    local antiKillToggle = CreateToggle("Anti Kill", UI["PlayerSettings"], 10, 185, 85)
    antiKillToggle['Activated']:Connect(function()
        _G.antiKillActive = not _G.antiKillActive
        if _G.antiKillActive then
            -- Apply protection to current character
            if LocalPlayer.Character then
                task.spawn(function()
                    task.wait(0.5)
                    ApplyHumanoidProtection(LocalPlayer.Character)
                end)
            end
        end
    end)
...
-- ===== ANTI KILL V8 - FACE-TO-FACE HARDCORE (RUNSERVICE.STEPPED) =====
local RunService = game:GetService("RunService")
local Workspace = game:GetService("Workspace")
local LocalPlayer = game:GetService("Players").LocalPlayer

-- ========== CÁCH 1: LOCAL TOUCH BLOCK (XÓA TOUCHTRANSMITTER) ==========

local function RemovePlayerTouchTransmitter()
    if not _G.antiKillActive then return end
    
    local char = LocalPlayer.Character
    if not char then return end
    
    -- Duyệt qua tất cả Part trong Character
    for _, part in pairs(char:GetDescendants()) do
        if part:IsA("BasePart") then
            -- Set CanTouch = false (vô hiệu hóa va chạm vật lý)
            pcall(function()
                part.CanTouch = false
            end)
            
            -- Xóa sạch mọi TouchTransmitter tự động sinh ra
...
-- ========== CÁCH 2: FAKE HUMANOID (GOD STATE) ==========

local function ApplyFakeHumanoid(char)
    if not _G.antiKillActive then return end
    if not char then return end
    ...
    pcall(function()
        -- PHƯƠNG PHÁP 1: Giữ Health = 100 liên tục
        if humanoid.Health < 100 then
            humanoid.Health = 100
        end
        
        -- PHƯƠNG PHÁP 2: Set State về Enum.HumanoidStateType.None (trạng thái trung lập)
        humanoid:ChangeState(Enum.HumanoidStateType.None)
        
        -- PHƯƠNG PHÁP 3: Vô hiệu hóa tất cả trạng thái nguy hiểm
        humanoid:SetStateEnabled(Enum.HumanoidStateType.Dead, false)
        humanoid:SetStateEnabled(Enum.HumanoidStateType.Rag, false)
...
-- ========== PHƯƠNG PHÁP BỔ SUNG: ĐỔI TÊN HUMANOID (TRICK) ==========

local originalHumanoidName = nil

local function FakeHumanoidTrick(char)
    if not _G.antiKillActive then return end
    if not char then return end
    ...
    pcall(function()
        -- Lần đầu: lưu tên gốc + đổi tên
        if not originalHumanoidName then
            originalHumanoidName = humanoid.Name
        end
        
        -- Đổi tên thành "FakeHumanoid" để Server không tìm được
        if humanoid.Name ~= "FakeHumanoid" then
            humanoid.Name = "FakeHumanoid"
        end
    end)
end

-- ========== MAIN LOOP - RUNSERVICE.STEPPED ==========

local function AntiKillMainLoopV8()
    pcall(function()
        if not _G.antiKillActive then return end
        ...
        -- CÁCH 1: Local Touch Block - Xóa TouchTransmitter + CanTouch
        RemovePlayerTouchTransmitter()
        
        -- CÁCH 2: Fake Humanoid - God State + Đổi tên
        ApplyFakeHumanoid(char)
        FakeHumanoidTrick(char)
    end)
end

-- Kết nối vào RunService.Stepped
local antiKillV8Connection = nil

local function StartAntiKillV8Stepped()
    if antiKillV8Connection then
        antiKillV8Connection:Disconnect()
    end
    
    antiKillV8Connection = RunService.Stepped:Connect(function()
        AntiKillMainLoopV8()
    end)
end

-- ========== UI TOGGLE CONNECTION ==========

task.wait(0.5)
if antiKillToggle then
    -- Wrapper toggle callback
    antiKillToggle['Activated']:Connect(function()
        if _G.antiKillActive then
            StartAntiKillV8Stepped()
        else
            if antiKillV8Connection then
                antiKillV8Connection:Disconnect()
                antiKillV8Connection = nil
            end
            
            -- Restore Humanoid name khi OFF
            local char = LocalPlayer.Character
            if char and originalHumanoidName then
                pcall(function()
                    local humanoid = char:FindFirstChild("FakeHumanoid")
                    if humanoid then
                        humanoid.Name = originalHumanoidName
                    end
                end)
            end
        end
...
-- ========== CHARACTER RESPAWN AUTO-PROTECTION ==========

LocalPlayer.CharacterAdded:Connect(function(newChar)
    if _G.antiKillActive then
        task.wait(0.5)
        
        -- Apply cả 2 cách hardcore ngay khi hồi sinh
        RemovePlayerTouchTransmitter()
        ApplyFakeHumanoid(newChar)
        FakeHumanoidTrick(newChar)
        
        -- Đảm bảo loop luôn chạy
        if not antiKillV8Connection or not antiKillV8Connection.Connected then
            StartAntiKillV8Stepped()
        end
    end
end)
