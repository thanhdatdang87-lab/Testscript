...
if (GameState == "MainGame") then
    ...
    -- ===== BOTTOM TRACER SYSTEM - MULTI-TARGET =====
    ...
    local function UpdateTracerLine(tracerData)
        ...
        local screenSize = Workspace.CurrentCamera.ViewportSize
        local bottomCenterPos = Vector2.new(screenSize.X / 2, screenSize.Y)
        
        line.From = bottomCenterPos
    ...
    local ghostTracerColorSlider = CreateSlider("Ghost Tracer Color", UI["PlayerSettings"], 11, 185, false, 0, 9, "GhostTracerColor")
    local equipmentTracerColorSlider = CreateSlider("Equipment Tracer Color", UI["PlayerSettings"], 12, 185, false, 0, 9, "EquipmentTracerColor")
    local cursedTracerColorSlider = CreateSlider("Cursed Tracer Color", UI["PlayerSettings"], 13, 185, false, 0, 9, "CursedTracerColor")
    local evidenceTracerColorSlider = CreateSlider("Evidence Tracer Color", UI["PlayerSettings"], 14, 185, false, 0, 9, "EvidenceTracerColor")
    ...
    ghostTracerColorSlider.Changed:Connect(function(value)
        local colorIndex = math.floor(tonumber(value) or 0) + 1
        BottomTracerConfig.ObjectTypes.Ghost.Color = tracerColors[math.max(1, math.min(10, colorIndex))]
    end)
    
    equipmentTracerColorSlider.Changed:Connect(function(value)
        local colorIndex = math.floor(tonumber(value) or 0) + 1
        BottomTracerConfig.ObjectTypes.Equipment.Color = tracerColors[math.max(1, math.min(10, colorIndex))]
    end)
    
    cursedTracerColorSlider.Changed:Connect(function(value)
        local colorIndex = math.floor(tonumber(value) or 0) + 1
        BottomTracerConfig.ObjectTypes.CursedObject.Color = tracerColors[math.max(1, math.min(10, colorIndex))]
    end)
    
    evidenceTracerColorSlider.Changed:Connect(function(value)
        local colorIndex = math.floor(tonumber(value) or 0) + 1
        BottomTracerConfig.ObjectTypes.Evidence.Color = tracerColors[math.max(1, math.min(10, colorIndex))]
    end)
