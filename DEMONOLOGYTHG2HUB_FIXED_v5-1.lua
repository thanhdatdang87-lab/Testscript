        ...
        for _, item in pairs(child_21:GetChildren()) do
            if item:IsA("Model") and item.PrimaryPart then
                ...
                if not isHeld then
                    ...
                    -- Handle cả Model và Part
                    if item:IsA("Model") then
                        primaryPart = item.PrimaryPart
                        if not primaryPart then
                            -- Nếu Model không có PrimaryPart, lấy first child là part
                    ...
                    while retries < maxRetries do
                        local targetPos = myPos + baseOffset + Vector3.new(0, heightOffset, 0)
                        ...
                        -- Check stuck
                        task.wait(0.05)
                        local checkRay = workspace:Raycast(primaryPart.Position + Vector3.new(0, 0.5, 0), Vector3.new(0, -1, 0))
