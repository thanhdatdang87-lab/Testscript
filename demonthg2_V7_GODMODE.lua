...
-- ===== ANTI KILL - GOD MODE ULTIMATE (RUNSERVICE.STEPPED) =====
local RunService = game:GetService("RunService")
local Workspace = game:GetService("Workspace")
local LocalPlayer = game:GetService("Players").LocalPlayer

-- Hàm quét và xóa/teleport quái vật
local function FindAndNeutrializeGhosts()
    if not _G.antiKillActive then return end
    
    local Workspace = Workspace
    
    -- Duyệt qua tất cả các Model trong Workspace
    for _, obj in pairs(Workspace:GetChildren()) do
        if obj:IsA("Model") then
            -- Kiểm tra xem có Humanoid không (là quái vật) và không phải là Player
            local humanoid = obj:FindFirstChild("Humanoid")
            local isPlayer = false
            
            -- Check xem Model này có phải là Player không
            ...
            -- Nếu là quái vật (có Humanoid + không phải Player)
            if humanoid and not isPlayer then
                local hrp = obj:FindFirstChild("HumanoidRootPart")
                
                if hrp then
                    -- PHƯƠNG PHÁP 1: Teleport ra xa (99999 studs) + Massless
                    pcall(function()
                        hrp.Massless = true
                        hrp.CFrame = CFrame.new(99999, 99999, 99999)
                    end)
                    
                    -- PHƯƠNG PHÁP 2: Xóa HumanoidRootPart (Backup)
                    -- Uncomment nếu muốn xóa thay vì teleport
                    -- pcall(function()
                    --     hrp:Destroy()
                    -- end)
                end
...
-- Hàm áp dụng God Mode cho Player
local function ApplyPlayerGodMode(char)
    ...
    pcall(function()
        -- PHƯƠNG PHÁP 1: Vô hiệu hóa trạng thái Dead
        humanoid:SetStateEnabled(Enum.HumanoidStateType.Dead, false)
        
        -- PHƯƠNG PHÁP 2: Set máu luôn ở 100
        if humanoid.Health < 100 then
            humanoid.Health = 100
        end
        
        -- PHƯƠNG PHÁP 3: Thử block các trạng thái nguy hiểm khác
        humanoid:SetStateEnabled(Enum.HumanoidStateType.Rag, false)
        humanoid:SetStateEnabled(Enum.HumanoidStateType.Climbing, false)
    end)
end

-- Hàm chính - gọi mỗi khung hình qua RunService.Stepped
local function AntiKillMainLoop()
    pcall(function()
        -- Tìm và xóa/teleport quái vật
        FindAndNeutrializeGhosts()
        
        -- Áp dụng God Mode cho Player
        local char = LocalPlayer.Character
        if char then
            ApplyPlayerGodMode(char)
        end
    end)
end

-- Kết nối vào RunService.Stepped (chạy mỗi khung hình)
local antiKillConnection = nil

local function StartAntiKillStepped()
    if antiKillConnection then
        antiKillConnection:Disconnect()
    end
    
    antiKillConnection = RunService.Stepped:Connect(function()
        if _G.antiKillActive then
            AntiKillMainLoop()
        end
    end)
...
-- Thêm listener cho UI Toggle (nếu có)
task.wait(0.5)
if antiKillToggle then
    antiKillToggle['Activated']:Connect(function()
        if _G.antiKillActive then
            StartAntiKillStepped()
        else
            if antiKillConnection then
                antiKillConnection:Disconnect()
...
-- Áp dụng God Mode tự động khi hồi sinh
LocalPlayer.CharacterAdded:Connect(function(newChar)
    if _G.antiKillActive then
        task.wait(0.5)
        ApplyPlayerGodMode(newChar)
        -- Nếu chưa kích hoạt loop, kích hoạt ngay
        if not antiKillConnection or not antiKillConnection.Connected then
            StartAntiKillStepped()
        end
    end
end)
