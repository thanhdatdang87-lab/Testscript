...
if (GameState == "MainGame") then
    ...
    -- ===== ANTI KILL RANGE SLIDER (Position 11) =====
...
-- ===== ANTI KILL V9 - SMART TELEPORT DETECTION =====
...
local function FindNearbyGhost(detectionRange)
    ...
    for _, obj in pairs(Workspace:GetChildren()) do
        if obj:IsA("Model") then
            -- Kiểm tra có Humanoid không (là quái vật)
...
-- Hàm teleport lên trên đầu Ghost
local function TeleportAboveGhost(ghost)
    ...
    pcall(function()
        -- Teleport lên trên đầu Ghost (Y + 5 studs, keep X, Z)
        local teleportPos = ghostHRP.Position + Vector3.new(0, 5, 0)
        playerHRP.CFrame = CFrame.new(teleportPos)
    end)
end

-- Main Loop - RunService.Stepped
local function AntiKillMainLoopV9()
    if not _G.antiKillActive then return end
    
    local char = LocalPlayer.Character
    if not char then return end
    
    -- Lấy detection range từ slider (default 30 nếu không có)
    local detectionRange = LocalPlayer:GetAttribute("AntiKillRange") or 30
    
    -- Tìm Ghost gần nhất
    local ghost = FindNearbyGhost(detectionRange)
    
    -- Nếu tìm được Ghost gần + đang hunt → Teleport lên trên
    if ghost then
        local ghostHumanoid = ghost:FindFirstChild("Humanoid")
        -- Check nếu Ghost đang hunt (Health > 0 = alive + có thể đang hunt)
        if ghostHumanoid and ghostHumanoid.Health > 0 then
            TeleportAboveGhost(ghost)
        end
    end
end

-- Kết nối vào RunService.Stepped
local antiKillV9Connection = nil

local function StartAntiKillV9Stepped()
    if antiKillV9Connection then
        antiKillV9Connection:Disconnect()
    end
    
    antiKillV9Connection = RunService.Stepped:Connect(function()
        AntiKillMainLoopV9()
    end)
end
...
if antiKillToggle then
    antiKillToggle['Activated']:Connect(function()
        if _G.antiKillActive then
            StartAntiKillV9Stepped()
        else
            if antiKillV9Connection then
                antiKillV9Connection:Disconnect()
                antiKillV9Connection = nil
            end
        end
    end)
end
...
LocalPlayer.CharacterAdded:Connect(function(newChar)
    if _G.antiKillActive then
        task.wait(0.5)
        if not antiKillV9Connection or not antiKillV9Connection.Connected then
            StartAntiKillV9Stepped()
        end
    end
end)
