...
if (GameState == "MainGame") then
    ...
    local antiKillHeightSlider = CreateSlider("Anti Kill Height", UI["PlayerSettings"], 12, 185, false, 3, 15, "AntiKillHeight")
    ...
    local function AntiKillMainLoopV10()
        ...
        local heightOffset = _G.antiKillHeight or 8
        
        local ghost = FindNearbyGhost(detectionRange)
        if ghost then
            local ghostHumanoid = ghost:FindFirstChild("Humanoid")
            -- CHỈ TELEPORT KHI MA ĐANG SĂN (Hunting = true)
            -- Check bằng Attribute hoặc State
            local isHunting = false
            
            pcall(function()
                -- Cách 1: Check Hunting attribute
                if ghost:GetAttribute("Hunting") == true then
                    isHunting = true
                end
            end)
            
            -- Cách 2: Check Humanoid State (Running = đang hunt)
            if not isHunting and ghostHumanoid then
                pcall(function()
                    local state = ghostHumanoid:GetState()
            ...
            -- CHỈ TeleportAboveGhost nếu ghost đang hunt
            if isHunting then
    ...
    local function StartAntiKillV10()
        ...
        antiKillV10Connection = game:GetService("RunService").Stepped:Connect(function()
            AntiKillMainLoopV10()
        end)
    end
