...
if (GameState == "MainGame") then
    ...
    -- ===== GHOST HUNTING PATTERN AI =====
    local huntHistory = {}
    local HuntPatternLabel = CreateLog("Hunt Pattern: Collecting data...", UI["GhostTabInfo1"], 1.5, 185, false, nil, false, false)
    local HuntPredictionLabel = CreateLog("Next Hunt: Unknown", UI["GhostTabInfo1"], 2, 185, false, nil, false, false)
    local HuntConfidenceLabel = CreateLog("Confidence: 0%", UI["GhostTabInfo1"], 2.5, 185, false, nil, false, false)
    
    local lastHuntState = false
    local currentHuntStart = nil
    
    -- Difficulty thresholds (từ thông tin bạn cung cấp)
    local difficultyThresholds = {
        ["Easy"] = 25,
        ["Medium"] = 50,
        ["Hard"] = 50,
        ["Nightmare"] = 75
    ...
    local function GetCurrentDifficulty()
        -- Detect difficulty từ game (fallback to Medium)
        return "Medium"  -- TODO: Detect từ game settings nếu có API
    end
    
    local function CalculateHuntPattern()
        if #huntHistory < 2 then
            return nil
        end
        
        -- Calculate intervals between hunts
        local intervals = {}
        for i = 2, #huntHistory do
        ...
        local patternType = "UNKNOWN"
        local confidence = 0
        
        if variance < avgInterval * 0.1 then
            patternType = "TIMED"
            confidence = 95
        elseif variance < avgInterval * 0.3 then
            patternType = "SEMI-STABLE"
            confidence = 75
        else
            patternType = "RANDOM"
            confidence = 40
        ...
        return {
            ...
            lastHunt = huntHistory[#huntHistory]
        }
    end
    
    local function PredictNextHunt(currentSanity, sanityDropRate)
        local data = CalculateHuntPattern()
        if not data then return nil end
        
        local difficulty = GetCurrentDifficulty()
        local threshold = difficultyThresholds[difficulty] or 50
        
        -- Method 1: Timed prediction
        local timedPrediction = data.lastHunt.endTime + data.avgInterval
        local timeUntilHunt = timedPrediction - tick()
        
        -- Method 2: Sanity-based prediction
        local sanityUntilHunt = currentSanity - (threshold - 5)  -- Buffer 5%
        local sanityPrediction = sanityUntilHunt / math.max(sanityDropRate, 0.1)
        
        -- Pick most conservative prediction
        local finalPrediction = math.min(timeUntilHunt, sanityPrediction)
        
        return {
            timeSeconds = finalPrediction,
            pattern = data.pattern,
            confidence = data.confidence,
            method = timeUntilHunt < sanityPrediction and "TIMED" or "SANITY",
    ...
    local function UpdateHuntAI()
        ...
        local data = CalculateHuntPattern()
        if data then
            HuntPatternLabel['Text'] = "Hunt Pattern: " .. data.pattern
            
            -- Color code confidence
            if data.confidence > 80 then
                HuntPatternLabel['TextColor3'] = Color3.fromRGB(0, 255, 0)  -- 🟢 Green
            elseif data.confidence > 60 then
                HuntPatternLabel['TextColor3'] = Color3.fromRGB(255, 200, 0)  -- 🟡 Yellow
            else
                HuntPatternLabel['TextColor3'] = Color3.fromRGB(255, 0, 0)  -- 🔴 Red
            end
            
            HuntConfidenceLabel['Text'] = string.format("Confidence: %d%%", data.confidence)
        end
        
        -- Update prediction
        local currentSanity = tonumber(LocalPlayer:GetAttribute("Sanity") or 100)
        local pred = PredictNextHunt(currentSanity, sanityDropRate)
        
        if pred and pred.timeSeconds > 0 then
            local minutes = math.floor(pred.timeSeconds / 60)
            local seconds = math.floor(pred.timeSeconds % 60)
            
            HuntPredictionLabel['Text'] = string.format("Next Hunt: %d:%02d (%s)", minutes, seconds, pred.method)
            
            -- Color code urgency
            if pred.timeSeconds < 60 then
                HuntPredictionLabel['TextColor3'] = Color3.fromRGB(255, 0, 0)  -- 🔴 Critical
            elseif pred.timeSeconds < 180 then
                HuntPredictionLabel['TextColor3'] = Color3.fromRGB(255, 165, 0)  -- 🟠 Warning
            else
                HuntPredictionLabel['TextColor3'] = Color3.fromRGB(0, 255, 0)  -- 🟢 Safe
            end
        else
            HuntPredictionLabel['Text'] = "Next Hunt: Calculating..."
        end
    end
