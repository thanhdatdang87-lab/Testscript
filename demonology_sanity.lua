...
if (GameState == "MainGame") then
    ...
    -- ===== SANITY MONITOR =====
    _G.sanityMonitorActive = false
    
    local sanityToggle = CreateToggle("Sanity Monitor", UI["PlayerSettings"], 10, 185, 85)
    
    -- Create draggable + resizable GUI
    local sanityFrame = nil
    local sanityConnection = nil
    
    local function CreateSanityGUI()
        -- Main Frame
        sanityFrame = Instance.new("ScreenGui")
        sanityFrame.Name = "SanityMonitor"
        sanityFrame.ResetOnSpawn = false
        sanityFrame.Parent = game.Players.LocalPlayer:WaitForChild("PlayerGui")
        ...
        game:GetService("UserInputService").InputChanged:Connect(function(input, gameProcessed)
            if dragging and dragStart then
                ...
                bg.Position = bg.Position + UDim2.new(0, delta.X, 0, delta.Y)
                dragStart = input.Position
            end
        end)
        ...
        game:GetService("UserInputService").InputChanged:Connect(function(input, gameProcessed)
            if resizing and resizeStart then
                ...
                bg.Size = UDim2.new(0, math.max(200, bg.Size.X.Offset + delta.X), 0, math.max(150, bg.Size.Y.Offset + delta.Y))
                resizeStart = input.Position
            end
        end)
        ...
        local function UpdateSanity()
            ...
            for _, player in pairs(players) do
                local sanity = player:GetAttribute("Sanity") or 100
                
                local playerLabel = Instance.new("TextLabel")
                playerLabel.Name = player.Name
                playerLabel.Text = "remaining sanity of " .. player.Name .. ": " .. math.floor(sanity)
                playerLabel.Size = UDim2.new(1, 0, 0, 25)
                playerLabel.Position = UDim2.new(0, 0, 0, yPos)
                playerLabel.BackgroundTransparency = 0.5
                playerLabel.BackgroundColor3 = Color3.fromRGB(50, 50, 70)
        ...
        -- Update mỗi 0.5s
        sanityConnection = game:GetService("RunService").Heartbeat:Connect(function()
            if not _G.sanityMonitorActive or not sanityFrame then
    ...
    -- Toggle Connection
    sanityToggle['Activated']:Connect(function()
        _G.sanityMonitorActive = not _G.sanityMonitorActive
        if _G.sanityMonitorActive then
            if sanityFrame then
                sanityFrame:Destroy()
            end
            CreateSanityGUI()
