...
if (GameState == "MainGame") then
    ...
    -- ╔════════════════════════════════════════════════════════╗
    -- ║  TELE ALL ITEMS - AUTO FARM (SIMPLIFIED & WORKING)    ║
    -- ╚════════════════════════════════════════════════════════╝
    ...
    local function startAutoFarm()
        task.spawn(function()
            farmState.isRunning = true
            local character = LocalPlayer.Character
            local hrp = character:FindFirstChild("HumanoidRootPart")
            local startPos = hrp.CFrame
            
            -- Khai báo beforehand để tránh lỗi scope
            local droppedPositions = {}
            local isFirstRun = true
            
            -- MAIN LOOP - LIÊN TỤC CHO ĐẾN HẾT ITEMS
            while farmState.isRunning and character do
                -- VỨT items (nếu không phải lần đầu)
                ...
                -- QUÉT ITEMS CHƯA VỨT
                local items = {}
                for _, model in pairs(workspace:FindFirstChild("Items"):GetChildren()) do
                    if model:IsA("Model") and tonumber(model.Name) then
                        local part = model:FindFirstChildWhichIsA("BasePart")
                        local prompt = model:FindFirstChildWhichIsA("ProximityPrompt", true)
                        
                        if part and prompt and prompt.Enabled then
                            -- Check xem đã vứt chưa
                            local wasDropped = false
                            for _, pos in pairs(droppedPositions) do
                                if (part.Position - pos).Magnitude < 8 then
                                    wasDropped = true
                                    break
                                end
                            end
                            
                            if not wasDropped then
                ...
                for i = 1, grabCount do
                    if not farmState.isRunning then break end
                    ...
                    -- TELE tới item
                    character:PivotTo(CFrame.new(target.Position + Vector3.new(0, 1, 0)))
                    task.wait(0.15)
                    
                    -- XOAY CAMERA
                    Camera.CameraType = Enum.CameraType.Scriptable
                    Camera.CFrame = CFrame.new(Camera.Position, target.Position)
                    task.wait(0.1)
                ...
                -- TELE VỀ ĐIỂM BẮT ĐẦU
                hrp = character:FindFirstChild("HumanoidRootPart")
                if hrp then
                    character:PivotTo(startPos)
                    task.wait(0.2)
                    table.insert(droppedPositions, startPos.Position)
    ...
    -- [CREATE SUB BUTTON]
    local function createSubButton()
        if farmState.subButtonCreated then return end
        farmState.subButtonCreated = true
        ...
        btn.MouseButton1Click:Connect(function()
            startAutoFarm()
            gui:Destroy()
            farmState.subButtonCreated = false
        end)
