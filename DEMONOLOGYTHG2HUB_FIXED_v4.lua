        ...
        for _, item in pairs(child_21:GetChildren()) do
            if item:IsA("Model") and item.PrimaryPart then
                ...
                if not isHeld then
                    ...
                    local targetPos = myPos + baseOffset
                    local heightOffset = 1.5
                    local maxRetries = 3
                    local retries = 0
                    
                    -- Try teleport, nếu stuck thì try lại với cao hơn
                    while retries < maxRetries do
                        targetPos = myPos + baseOffset + Vector3.new(0, heightOffset, 0)
                        
                        -- Raycast check để tránh terrain
                        local rayHit = workspace:Raycast(targetPos + Vector3.new(0, 2, 0), Vector3.new(0, -4, 0))
                        if rayHit then
                            targetPos = rayHit.Position + Vector3.new(0, 1, 0)
                        end
                        ...
                        if item.PrimaryPart then
                            ...
                            item.PrimaryPart.AssemblyAngularVelocity = Vector3.new(0, 0, 0)
                            item.PrimaryPart.CanCollide = true
                        end
                        
                        -- Check xem item còn stuck không
                        task.wait(0.05)
                        local checkRay = workspace:Raycast(item.PrimaryPart.Position + Vector3.new(0, 0.5, 0), Vector3.new(0, -1, 0))
                        if not checkRay or checkRay.Distance > 0.3 then
                            -- Item not stuck, break loop
                            break
                        else
                            -- Item still stuck, try higher
