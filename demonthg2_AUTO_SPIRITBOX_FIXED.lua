        ...
        if autoSpiritActive then
            ...
            task.spawn(function()
                while autoSpiritActive do
                    ...
                    -- ✅ Nếu ghost không hunt, tele gần ghost
                    if ghostModel:GetAttribute("Hunting") ~= true then
                        Chara:PivotTo(ghostModel:GetPivot() * CFrame.new(0, 0, 10))
                    else
                        -- ✅ Nếu ghost hunt, tele ra ngoài
                        if Chara and Chara:FindFirstChild("HumanoidRootPart") then
                            Chara:PivotTo(CFrame.new(300, 100, 300))  -- Far away safe spot
                            task.wait(0.5)
                        end
                        continue
                    end
                    
                    -- ✅ Tìm Spirit Box trong workspace items
                    local spiritBoxItem = nil
                    ...
                    -- ✅ Tìm Spirit Box trong inventory (equippable)
                    local spiritBoxEquipped = Chara:FindFirstChild("Spirit Box")
                    
                    if spiritBoxItem then
                        -- ✅ Tele tới spirit box item
                        Chara:PivotTo(spiritBoxItem:GetPivot() + Vector3.new(0, 3, 0))
                        task.wait(0.3)
                    ...
                    -- ✅ Equip + Activate Spirit Box
                    if spiritBoxEquipped or spiritBoxItem then
                        -- Equip từ inventory hoặc cầm trực tiếp
                        if spiritBoxEquipped then
                            spiritBoxEquipped:Activate()
                            task.wait(0.3)
                        end
                        
                        -- ✅ Spam hỏi spirit box (press E nhiều lần)
                        for i = 1, 5 do
                            if not autoSpiritActive then break end
                            VirtualInputManager:SendKeyEvent(true, Enum.KeyCode.E, false, game)
                            task.wait(0.15)
                            VirtualInputManager:SendKeyEvent(false, Enum.KeyCode.E, false, game)
                            task.wait(0.35)
                    ...
                    task.wait(1)  -- Delay trước lần tiếp theo
                end
